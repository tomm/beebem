...
static unsigned char
    Internal_BadTracks[2][2]; /* 1st subscript is surface 0/1 and second
                                 subscript is badtrack 0/1 */
...
static int NextInterruptIsErr; /* none 0 causes error and drops this value into
                                  result reg */
...
/* All data on the disc - first param is drive number, then head. then physical
 * track id */
...
typedef struct {
  ...
  unsigned char Mask; /* Mask command with this before comparing with CommandNum
                         - allows drive ID to be removed */
  int NParams;        /* Number of parameters to follow */
  CommandFunc ToCall; /* Called after all paameters have arrived */
  CommandFunc IntHandler; /* Called when interrupt requested by command is about
                             to happen */
...
static SectorType *GetSectorPtr(TrackType *Track, int LogicalSectorID,
                                int FindDeleted) {
  int CurrentSector;
  ...
  for (CurrentSector = 0; CurrentSector < Track->NSectors; CurrentSector++)
    ...
    //++
    if ((Track->Sectors[CurrentSector].IDField.RecordNum ==
         (unsigned int)LogicalSectorID) &&
        ((!Track->Sectors[CurrentSector].Deleted) || (!FindDeleted)))
      //<-
...
static void DoVarLength_WriteDataCommand(void) {
  ...
  CommandStatus.CurrentSectorPtr =
      GetSectorPtr(CommandStatus.CurrentTrackPtr, Params[1], 0);
  ...
  if (ValidateSector(CommandStatus.CurrentSectorPtr, CommandStatus.TrackAddr,
  ...
  if (CommandStatus.ByteWithinSector >= CommandStatus.SectorLength) {
    ...
    if (--CommandStatus.SectorsToGo) {
      ...
      CommandStatus.CurrentSectorPtr = GetSectorPtr(
          CommandStatus.CurrentTrackPtr, CommandStatus.CurrentSector, 0);
      if (CommandStatus.CurrentSectorPtr == NULL) {
        DoErr(0x1e); /* Sector not found */
        return;
      }
...
static void ReadInterrupt(void) {
  ...
  if (CommandStatus.ByteWithinSector >= CommandStatus.SectorLength) {
    CommandStatus.ByteWithinSector = 0;
    /* I don't know if this can cause the thing to step - I presume not for the
     * moment */
    if (--CommandStatus.SectorsToGo) {
      CommandStatus.CurrentSector++;
      if (CommandStatus.CurrentSectorPtr = GetSectorPtr(
              CommandStatus.CurrentTrackPtr, CommandStatus.CurrentSector, 0),
...
static void DoReadIDCommand(void) {
  ...
  CommandStatus.CurrentSectorPtr =
      GetSectorPtr(CommandStatus.CurrentTrackPtr, 0, 0);
  if (CommandStatus.CurrentSectorPtr == NULL) {
    DoErr(0x1e); /* Sector not found */
    return;
  };
...
static void DoReadDriveStatusCommand(void) {
  ...
  ResultReg = 0x80 | (Selects[1] ? 0x40 : 0) | (Selects[0] ? 0x4 : 0) |
              (Track0 ? 2 : 0) | (WriteProt ? 8 : 0);
...
/*--------------------------------------------------------------------------*/
/* The following table is used to parse commands from the command number written
into the command register - it can't distinguish between subcommands selected
from the first parameter */
static PrimaryCommandLookupType PrimaryCommandLookup[] = {
    {0x00, 0x3f, 3, DoVarLength_ScanDataCommand, NULL,
    ...
    {0x0a, 0x3f, 2, Do128ByteSR_WriteDataCommand, NULL,
     "Write Data (128 byte/single record)"},
    {0x0b, 0x3f, 3, DoVarLength_WriteDataCommand, WriteInterrupt,
     "Write Data (Variable Length/Multi-Record)"},
    {0x0e, 0x3f, 2, Do128ByteSR_WriteDeletedDataCommand, NULL,
     "Write Deleted Data (128 byte/single record)"},
    {0x0f, 0x3f, 3, DoVarLength_WriteDeletedDataCommand, NULL,
     "Write Deleted Data (Variable Length/Multi-Record)"},
    {0x12, 0x3f, 2, Do128ByteSR_ReadDataCommand, NULL,
     "Read Data (128 byte/single record)"},
    {0x13, 0x3f, 3, DoVarLength_ReadDataCommand, ReadInterrupt,
     "Read Data (Variable Length/Multi-Record)"},
    {0x16, 0x3f, 2, Do128ByteSR_ReadDataAndDeldCommand, NULL,
    ...
    {0, 0, 0, DoBadCommand, NULL,
     "Unknown command"} /* Terminator due to 0 mask matching all */
...
/*--------------------------------------------------------------------------*/
void LoadSimpleDiscImage(char *FileName, int DriveNum, int HeadNum,
                         int Tracks) {
  ...
  for (CurrentTrack = 0; CurrentTrack < Tracks; CurrentTrack++) {
    ...
    SecPtr = DiscStore[DriveNum][HeadNum][CurrentTrack].Sectors =
        (SectorType *)calloc(10, sizeof(SectorType));
    DiscStore[DriveNum][HeadNum][CurrentTrack].Gap1Size =
...
void LoadSimpleDSDiscImage(char *FileName, int DriveNum, int Tracks) {
  ...
  /* Check if the side 2 catalogue sectors look OK - give a warning if they do
   * not. */
...
/*--------------------------------------------------------------------------*/
int IsDiscWritable(int DriveNum) { return Writeable[DriveNum]; }
...
void CreateDiscImage(char *FileName, int DriveNum, int Heads, int Tracks) {
  ...
  if (outfile != NULL) {
    ...
    if (MessageBox(GETHWND, errstr, "BBC Emulator",
  ...
  if (scanfres = sscanf(DiscString, "%c:%d:%s", &DoubleSided, &Tracks, Name),
    ...
    case 'a':
      if ((MachineType == 3) || (!NativeFDC))
        ...
        MessageBox(GETHWND,
                   "The 8271 FDC Cannot load the ADFS disc image specified in "
                   "the BeebDiscLoad environment variable",
...
void Disc8271_reset(void) {
  ...
  Internal_ModeReg = 0;
  Internal_CurrentTrack[0] = Internal_CurrentTrack[1] =
      0; /* 0/1 for surface number */
  Internal_DriveControlOutputPort = 0;
  Internal_DriveControlInputPort = 0;
  Internal_BadTracks[0][0] = Internal_BadTracks[0][1] =
      Internal_BadTracks[1][0] = Internal_BadTracks[1][1] =
          0xff; /* 1st subscript is surface 0/1 and second subscript is badtrack
                   0/1 */
