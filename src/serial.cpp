...
You may not distribute this entire file separate from the whole BeebEm
distribution.
...
3) Acknowledgement is made to the author, and any aubsequent authors of
additional code.

The code may be modified as required, and freely distributed as such authors see
fit, provided that:

1) Acknowledgement is made to the author, and any subsequent authors of
additional code 2) Indication is made of modification.

Absolutely no warranties/guarantees are made by using this code. You use and/or
run this code at your own risk. The code is classed by the author as "unlikely
to cause problems as far as can be determined under normal use".

-- end of legal crap - Richard Gellman :) */

// P.S. If anybody knows how to emulate this, do tell me - 16/03/2001 - Richard
// Gellman
...
unsigned char DCD = 0, DCDI = 1, ODCDI = 1,
              DCDClear = 0; // count to clear DCD bit

unsigned char Parity, Stop_Bits, Data_Bits, RIE,
    TIE; // Receive Intterrupt Enable
         // and Transmit Interrupt Enable
unsigned char TxD,
    RxD; // Transmit and Receive destinations (data or shift register)
...
unsigned char SerialPortOpen =
    0; // Indicates status of serial port (on the host)
...
void SetACIAStatus(unsigned char bit) { ACIA_Status |= 1 << bit; }

void ResetACIAStatus(unsigned char bit) { ACIA_Status &= ~(1 << bit); }
...
void Write_ACIA_Tx_Data(unsigned char Data) {
   ...
   * Unless we do something with the data, the loader hangs so just swallow it
   * (see below)
   */

  if ((SerialChannel == CASSETTE) ||
      ((SerialChannel == RS423) && (!SerialPortEnabled))) {
    ResetACIAStatus(1);
    TDR = Data;
    TxD = 1;
...
void Write_SERPROC(unsigned char Data) {
  ...
  TapeAudio.Enabled =
      (Cass_Relay && (TapePlaying || TapeRecording)) ? TRUE : FALSE;
...
void Serial_Poll(void) {
  ...
  //	if (trace == 1) WriteLog("Here - SerialChannel = %d, TapeRecording =
  ...
  if ((SerialChannel == RS423) && (SerialPortEnabled)) {
    ...
    if (TouchScreenEnabled) {
      ...
      //(!bWaitingForData) && (RxD<2)) {
      //--				if
      //(!ReadFile(hSerialPort,&SerialBuffer,1,&BytesIn,&olSerialPort)) {
      //--					if
...
void StatThread(void *lpParam) {
  ...
  do {
    ...
    if (!TouchScreenEnabled) {
      ...
      //(SerialPortEnabled) ) {
      //--			if
      //(GetOverlappedResult(hSerialPort,&olStatus,&dwOvRes,FALSE)) {
...
void InitSerialPort(void) {
  ...
  if ((SerialPortEnabled) && (!TouchScreenEnabled)) {
    ...
    //--			bPortStat=GetCommState(hSerialPort,
    //&dcbSerialPort);
...
void SetUnlockTape(int unlock) { uef_setunlock(unlock); }
      ...
      if (UEFRES_TYPE(data) != UEFRES_TYPE(last_data)) {
        if (UEFRES_TYPE(last_data) == UEF_DATA) {
          ...
          if (blk > 20 && block[0] == 0x2A) {
            ...
            n = 1;
            while (block[n] != 0 && block[n] >= 32 && block[n] <= 127 &&
                   n <= 10) {
              name[n - 1] = block[n];
              n++;
            }
            name[n - 1] = 0;
            if (name[0] != 0)
              sprintf(map_desc[map_lines], "%-12s %02X  Length %04X", name,
                      blk_num, blk);
            else
              sprintf(map_desc[map_lines], "<No name>    %02X  Length %04X",
                      blk_num, blk);
            ...
            sprintf(map_desc[map_lines], "Non-standard %02X  Length %04X",
                    blk_num, blk);
            map_time[map_lines] = start_time;
...
//--		SendMessage(hwndMap, WM_SETFONT,
...
void TapeControlOpenFile(const char *UEFName) {
  ...
  if (TapeControlEnabled) {
    if (!map_file(UEFName)) {
      ...
      //--				SendMessage(hwndMap, LB_ADDSTRING, 0,
      //(LPARAM)map_desc[i]);
    }
  }
}
