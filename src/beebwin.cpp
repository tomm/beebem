...
bool DiscLoaded[2] = {FALSE,
                      FALSE}; // Set to TRUE when a disc image has been loaded.
...
void BeebWin::doHorizLine(unsigned long Col, int y, int sx, int width) {
  ...
  switch (IsFullScreen() ? cfg_Fullscreen_Resolution
...
BOOL BeebWin::InitClass(void) {
  ...
  //--	wc.lpszClassName = "BEEBWIN"; //szAppName;				//
  //Name to register as
  //--
...
void BeebWin::InitMenu(void) {
  ...
  CheckMenuItem(hMenu, IDM_SPEEDANDFPS,
                m_ShowSpeedAndFPS ? MF_CHECKED : MF_UNCHECKED);
  CheckMenuItem(hMenu, m_MenuIdWinSize, MF_CHECKED);
  CheckMenuItem(hMenu, IDM_FULLSCREEN,
                m_isFullScreen ? MF_CHECKED : MF_UNCHECKED);
  CheckMenuItem(hMenu, IDM_DD32ONOFF, m_isDD32 ? MF_CHECKED : MF_UNCHECKED);
  CheckMenuItem(hMenu, IDM_SOUNDONOFF,
                SoundEnabled ? MF_CHECKED : MF_UNCHECKED);
  ...
  CheckMenuItem(hMenu, IDM_FREEZEINACTIVE,
                m_FreezeWhenInactive ? MF_CHECKED : MF_UNCHECKED);
  CheckMenuItem(hMenu, IDM_HIDECURSOR,
                m_HideCursor ? MF_CHECKED : MF_UNCHECKED);
  ...
  CheckMenuItem(hMenu, IDM_WPDISC0,
                m_WriteProtectDisc[0] ? MF_CHECKED : MF_UNCHECKED);
  CheckMenuItem(hMenu, IDM_WPDISC1,
                m_WriteProtectDisc[1] ? MF_CHECKED : MF_UNCHECKED);
  CheckMenuItem(hMenu, IDM_AMXONOFF,
                AMXMouseEnabled ? MF_CHECKED : MF_UNCHECKED);
  CheckMenuItem(hMenu, IDM_AMX_LRFORMIDDLE,
                AMXLRForMiddle ? MF_CHECKED : MF_UNCHECKED);
  CheckMenuItem(hMenu, m_MenuIdAMXSize, MF_CHECKED);
  if (m_MenuIdAMXAdjust != 0)
    CheckMenuItem(hMenu, m_MenuIdAMXAdjust, MF_CHECKED);
  CheckMenuItem(hMenu, IDM_PRINTERONOFF,
                PrinterEnabled ? MF_CHECKED : MF_UNCHECKED);
  strcpy(menu_string, "File: ");
  strcat(menu_string, m_PrinterFileName);
  ModifyMenu(hMenu, IDM_PRINTER_FILE, MF_BYCOMMAND, IDM_PRINTER_FILE,
             menu_string);
  CheckMenuItem(hMenu, m_MenuIdPrinterPort, MF_CHECKED);
  CheckMenuItem(hMenu, IDM_DDRAWONOFF,
                m_DirectDrawEnabled ? MF_CHECKED : MF_UNCHECKED);
  ...
  CheckMenuItem(hMenu, IDM_TEXTVIEW,
                m_TextViewEnabled ? MF_CHECKED : MF_UNCHECKED);

  CheckMenuItem(hMenu, ID_SERIAL,
                SerialPortEnabled ? MF_CHECKED : MF_UNCHECKED);
  CheckMenuItem(hMenu, ID_TOUCHSCREEN,
                TouchScreenEnabled ? MF_CHECKED : MF_UNCHECKED);
  ...
  CheckMenuItem(hMenu, ID_HIDEMENU,
                HideMenuEnabled ? MF_CHECKED : MF_UNCHECKED);

  CheckMenuItem(m_hMenu, ID_TAPESOUND,
                (TapeSoundEnabled) ? MF_CHECKED : MF_UNCHECKED);
  CheckMenuItem(m_hMenu, IDM_SOUNDCHIP,
                (SoundChipEnabled) ? MF_CHECKED : MF_UNCHECKED);
  CheckMenuItem(m_hMenu, ID_PSAMPLES,
                (PartSamples) ? MF_CHECKED : MF_UNCHECKED);
  CheckMenuItem(m_hMenu, IDM_EXPVOLUME,
                SoundExponentialVolume ? MF_CHECKED : MF_UNCHECKED);
  CheckMenuItem(hMenu, IDM_TEXTTOSPEECH,
                m_TextToSpeechEnabled ? MF_CHECKED : MF_UNCHECKED);

  CheckMenuItem(m_hMenu, IDM_TUBE, (TubeEnabled) ? MF_CHECKED : MF_UNCHECKED);
  CheckMenuItem(m_hMenu, IDM_TUBE186,
                (Tube186Enabled) ? MF_CHECKED : MF_UNCHECKED);
  ...
  CheckMenuItem(m_hMenu, ID_UNLOCKTAPE,
                (UnlockTape) ? MF_CHECKED : MF_UNCHECKED);
  CheckMenuItem(m_hMenu, m_MotionBlur, MF_CHECKED);
#ifdef WITH_ECONET
  CheckMenuItem(hMenu, ID_ECONET,
                EconetEnabled ? MF_CHECKED : MF_UNCHECKED); // Rob
#endif
  CheckMenuItem(hMenu, ID_TELETEXT,
                TeleTextAdapterEnabled ? MF_CHECKED : MF_UNCHECKED);
  CheckMenuItem(hMenu, ID_HARDDRIVE,
                HardDriveEnabled ? MF_CHECKED : MF_UNCHECKED);
  CheckMenuItem(hMenu, IDM_SPEECH, SpeechDefault ? MF_CHECKED : MF_UNCHECKED);

  //+>
  // BeebEm SDL Additional Options:

  CheckMenuItem(hMenu, OPT_EMULATECRTGRAPHICS,
                cfg_EmulateCrtGraphics ? MF_CHECKED : MF_UNCHECKED);
  CheckMenuItem(hMenu, OPT_EMUALTECRTTELETEXT,
                cfg_EmulateCrtTeletext ? MF_CHECKED : MF_UNCHECKED);
  CheckMenuItem(hMenu, OPT_WANTLOWLATENCYSOUND,
                cfg_WantLowLatencySound ? MF_CHECKED : MF_UNCHECKED);
...
void BeebWin::UpdateMonitorMenu() {
  HMENU hMenu = m_hMenu;
  CheckMenuItem(hMenu, ID_MONITOR_RGB,
                (palette_type == RGB) ? MF_CHECKED : MF_UNCHECKED);
  CheckMenuItem(hMenu, ID_MONITOR_BW,
                (palette_type == BW) ? MF_CHECKED : MF_UNCHECKED);
  CheckMenuItem(hMenu, ID_MONITOR_GREEN,
                (palette_type == GREEN) ? MF_CHECKED : MF_UNCHECKED);
  CheckMenuItem(hMenu, ID_MONITOR_AMBER,
                (palette_type == AMBER) ? MF_CHECKED : MF_UNCHECKED);
}

void BeebWin::UpdateModelType() {
  HMENU hMenu = m_hMenu;
  CheckMenuItem(hMenu, ID_MODELB,
                (MachineType == 0) ? MF_CHECKED : MF_UNCHECKED);
  CheckMenuItem(hMenu, ID_MODELBINT,
                (MachineType == 1) ? MF_CHECKED : MF_UNCHECKED);
  CheckMenuItem(hMenu, ID_MODELBP,
                (MachineType == 2) ? MF_CHECKED : MF_UNCHECKED);
  CheckMenuItem(hMenu, ID_MASTER128,
                (MachineType == 3) ? MF_CHECKED : MF_UNCHECKED);
}

void BeebWin::UpdateSFXMenu() {
  HMENU hMenu = m_hMenu;
  CheckMenuItem(hMenu, ID_SFX_RELAY,
                RelaySoundEnabled ? MF_CHECKED : MF_UNCHECKED);
}

void BeebWin::UpdateDisableKeysMenu() {
  CheckMenuItem(m_hMenu, IDM_DISABLEKEYSWINDOWS,
                m_DisableKeysWindows ? MF_CHECKED : MF_UNCHECKED);
  CheckMenuItem(m_hMenu, IDM_DISABLEKEYSBREAK,
                m_DisableKeysBreak ? MF_CHECKED : MF_UNCHECKED);
  CheckMenuItem(m_hMenu, IDM_DISABLEKEYSESCAPE,
                m_DisableKeysEscape ? MF_CHECKED : MF_UNCHECKED);
  CheckMenuItem(m_hMenu, IDM_DISABLEKEYSSHORTCUT,
                m_DisableKeysShortcut ? MF_CHECKED : MF_UNCHECKED);
}
...
void BeebWin::InitDirectX(void) {
  ...
  //--	if( ddrval == DD_OK )
  //--	{
  //--		ddrval = m_DD->QueryInterface(IID_IDirectDraw2, (LPVOID
  //*)&m_DD2);
  //--	}
  //--	if( ddrval == DD_OK )
  //--	{
...
HRESULT BeebWin::InitSurfaces(void) {
  ...
  //--		{
  //--			if (m_isDD32)
  //--				ddrval = m_DD2->SetDisplayMode(m_XWinSize,
  //m_YWinSize, 32, 0, 0);
  //--			else
  //--				ddrval = m_DD2->SetDisplayMode(m_XWinSize,
  //m_YWinSize, 8, 0, 0);
  ...
  //--
  //--	if( ddrval == DD_OK )
  //--		ddrval = m_DDSPrimary->QueryInterface(IID_IDirectDrawSurface2,
  //(LPVOID *)&m_DDS2Primary);
  //--	if( ddrval == DD_OK )
...
void BeebWin::SetRomMenu(void) {
  ...
  for (i = 0; i < 16; i++) {
    ...
    /* LRW Now uncheck the Roms which are NOT writable, that have already been
     * loaded. */
    CheckMenuItem(hMenu, IDM_ALLOWWRITES_ROM0 + i,
                  RomWritable[i] ? MF_CHECKED : MF_UNCHECKED);
  }
}
...
void BeebWin::GetRomMenu(void) {
  ...
  for (int i = 0; i < 16; ++i)
     ...
     */
    RomWritable[i] =
        (GetMenuState(hMenu, IDM_ALLOWWRITES_ROM0 + i, MF_BYCOMMAND) &
         MF_CHECKED);
}
...
void BeebWin::SetMousestickButton(int button) {
  if ((m_MenuIdSticks == IDM_AMOUSESTICK) ||
      ((m_MenuIdSticks == IDM_DMOUSESTICK)))
...
//--					}
//--
//--					mainWin->TranslateKey((int)uParam,
//false, row, col);
//--				}
//--			}
//--			break;
...
void BeebWin::doLED(int sx, bool on) {
  int tsy;
  char colbase;
  colbase = (DiscLedColour * 2) +
            LED_COL_BASE; // colour will be 0 for red, 1 for green.
  if (sx < 100)
...
void BeebWin::updateLines(HDC hDC, int starty, int nlines) {
  ...
  //--			{
  //--				char fps[50];
  //--				sprintf(fps, "%2.2f %2d", m_RelativeSpeed,
  ...
  //--					if (ddrval == DD_OK)
  //--						ddrval = m_DDS2Primary->Blt(
  //&destRect, m_DDS2One, &srcRect, DDBLT_ASYNC, NULL );
  //--				}
  //--			}
  //--			else
  //--			{
  ...
  //--			// Ignore DX errors for now - swapping between full
  //screen and windowed DX
...
BOOL BeebWin::UpdateTiming(void) {
  ...
  /* Don't do anything if this is the first call or there has
     ...
     wraps. */
  if (m_LastTickCount == 0 || TickCount < m_LastTickCount ||
      (TickCount - m_LastTickCount) > 1000 || TotalCycles < m_LastTotalCycles) {
    ResetTiming();
    return TRUE;
  }
  ...
  if (m_RealTimeTarget > 0.0) {
    ...
    while ((TickCount - m_TickBase) > 1000 &&
...
void BeebWin::DisplayTiming(void) {
  if (m_ShowSpeedAndFPS && (!m_DirectDrawEnabled || !m_isFullScreen)) {
    sprintf(m_szTitle, "%s  Speed: %2.2f  fps: %2d", WindowTitle,
...
void BeebWin::AdjustSpeed(bool up) {
  static int speeds[] = {
      IDM_FIXEDSPEED100,  IDM_FIXEDSPEED50,   IDM_FIXEDSPEED10,
      IDM_FIXEDSPEED5,    IDM_FIXEDSPEED2,    IDM_FIXEDSPEED1_5,
      IDM_FIXEDSPEED1_25, IDM_FIXEDSPEED1_1,  IDM_REALTIME,
      IDM_FIXEDSPEED0_9,  IDM_FIXEDSPEED0_75, IDM_FIXEDSPEED0_5,
      IDM_FIXEDSPEED0_25, IDM_FIXEDSPEED0_1,  0};
  int s = 0;
...
int BeebWin::ReadDisc(int Drive, HMENU dmenu) {
  ...
  //--	ofn.hInstance = NULL;
  //--	ofn.lpstrFilter = "Auto
  //(*.ssd;*.dsd;*.ad*;*.img)\0*.ssd;*.dsd;*.adl;*.adf;*.img\0"
...
void BeebWin::NewDiscImage(int Drive) {
   ...
   */
  if (FileName[0] == '\0') {
    SysReg.GetStringValue(HKEY_CURRENT_USER, CFG_REG_KEY, "DiscsPath",
                          FileName);
  ...
  if (Save_GTK_File_Selector(FileName) == true) {
     ...
     */
    if (FileName[0] == '\0') {
      MessageBox(GETHWND, "No name given for file!", "Create floopy disc",
                 MB_OK | MB_ICONERROR);
      return;
    }
    ...
    if (FileName[strlen(FileName) - 1] == '/' ||
        FileName[strlen(FileName) - 1] == '\\') {
      MessageBox(GETHWND, "Cannot replace whole directory as disc image!",
...
void BeebWin::ToggleWriteProtect(int Drive) {
  ...
  if (MachineType == 3) {
    ...
    if (Drive == 0)
      CheckMenuItem(hMenu, IDM_WPDISC0,
                    DWriteable[0] ? MF_UNCHECKED : MF_CHECKED);
    else
      CheckMenuItem(hMenu, IDM_WPDISC1,
                    DWriteable[1] ? MF_UNCHECKED : MF_CHECKED);
  }
}
...
void BeebWin::SetDiscWriteProtects(void) {
  if (MachineType != 3) {
    m_WriteProtectDisc[0] = !IsDiscWritable(0);
    m_WriteProtectDisc[1] = !IsDiscWritable(1);
    CheckMenuItem(m_hMenu, IDM_WPDISC0,
                  m_WriteProtectDisc[0] ? MF_CHECKED : MF_UNCHECKED);
    CheckMenuItem(m_hMenu, IDM_WPDISC1,
                  m_WriteProtectDisc[1] ? MF_CHECKED : MF_UNCHECKED);
  } else {
    CheckMenuItem(m_hMenu, IDM_WPDISC0,
                  DWriteable[0] ? MF_UNCHECKED : MF_CHECKED);
...
void BeebWin::TogglePrinter() {
  ...
  //--					if
  //(MessageBox(m_hWnd,errstr,WindowTitle,MB_YESNO|MB_ICONQUESTION) != IDYES)
  //--						FileOK = FALSE;
  //--				}
  //--				if (FileOK == TRUE)
...
void BeebWin::LoadPreferences() {
  ...
  *binsize = 1;
  ...
  RegRes = SysReg.GetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "MachineType",
                                 &MachineType, binsize);
  if (!RegRes) {
    MachineType = 0;
  ...
  if (SysReg.GetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY,
                            CFG_VIEW_DIRECT_ENABLED, &flag, binsize)) {
  ...
  if (SysReg.GetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY,
                            CFG_VIEW_BUFFER_IN_VIDEO, &flag, binsize)) {
  ...
  //--	if
  //(SysReg.GetDWORDValue(HKEY_CURRENT_USER,CFG_REG_KEY,"DDFullScreenMode",dword))
  //{
  //--		m_DDFullScreenMode = dword;
  //--	}
  //--	else {
  //--		m_DDFullScreenMode = GetPrivateProfileInt(CFG_VIEW_SECTION,
  //"DDFullScreenMode", ID_VIEW_DD_640X480, CFG_FILE_NAME);
  //--	}
  //++
  ...
  //--	if
  //(SysReg.GetBinaryValue(HKEY_CURRENT_USER,CFG_REG_KEY,"isDD32",&flag,binsize))
  //{
  //--		m_isDD32 = flag;
  //--	}
  //--	else {
  ...
  if (SysReg.GetDWORDValue(HKEY_CURRENT_USER, CFG_REG_KEY, CFG_VIEW_WIN_SIZE,
                           dword)) {
  ...
  //--	else {
  //--		if
  //(SysReg.GetBinaryValue(HKEY_CURRENT_USER,CFG_REG_KEY,"isFullScreen",&flag,binsize))
  //{
  //--			m_isFullScreen = flag;
  //--		}
  //--		else {
  //--			m_isFullScreen = GetPrivateProfileInt(CFG_VIEW_SECTION,
  //"isFullScreen", 0, CFG_FILE_NAME);
  //--		}
  //--	}
  //++
  m_isFullScreen = false;
  if (SysReg.GetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "isFullScreen",
                            &flag, binsize))
    if (flag)
      ToggleFullscreen();
  //<-
  ...
  if (SysReg.GetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, CFG_VIEW_SHOW_FPS,
                            &flag, binsize)) {
    ...
    // nasty).
    GetPrivateProfileString(CFG_VIEW_SECTION, CFG_VIEW_SHOW_FPS, "0", CfgValue,
                            sizeof(CfgValue), CFG_FILE_NAME);
    //<+
    m_ShowSpeedAndFPS = atoi(CfgValue);
  }

  if (SysReg.GetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, CFG_VIEW_MONITOR,
                            &flag, binsize)) {
    palette_type = (PaletteType)flag;
  } else {
    GetPrivateProfileString(CFG_VIEW_SECTION, CFG_VIEW_MONITOR, "0", CfgValue,
                            sizeof(CfgValue), CFG_FILE_NAME);
    palette_type = PaletteType(atoi(CfgValue));
  }

  RegRes = SysReg.GetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY,
                                 "HideMenuEnabled", &HideMenuEnabled, binsize);
  if (!RegRes) {
    HideMenuEnabled = 0;
  }

  RegRes = SysReg.GetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY,
                                 "LED Information", &LEDByte, binsize);
  if (!RegRes) {
    LEDByte = 0;
  }
  ...
  if (SysReg.GetDWORDValue(HKEY_CURRENT_USER, CFG_REG_KEY, "MotionBlur",
                           dword)) {
    m_MotionBlur = dword;
  } else {
  ...
  *binsize = 8;
  if (!SysReg.GetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY,
                             "MotionBlurIntensities", m_BlurIntensities,
                             binsize)) {
  ...
  *binsize = 1;
  RegRes =
      SysReg.GetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "TextViewEnabled",
                            &m_TextViewEnabled, binsize);
  if (!RegRes) {
    m_TextViewEnabled = 0;
  }

  if (SysReg.GetDWORDValue(HKEY_CURRENT_USER, CFG_REG_KEY, CFG_SPEED_TIMING,
                           dword)) {
  ...
  RegRes = SysReg.GetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY,
                                 CFG_SOUND_ENABLED, &SoundDefault, binsize);
  if (!RegRes) {
    SoundDefault = 1;
  }

  RegRes =
      SysReg.GetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "SoundChipEnabled",
                            &SoundChipEnabled, binsize);
  if (!RegRes) {
    SoundChipEnabled = 1;
  }

  if (SysReg.GetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY,
                            CFG_SOUND_DIRECT_ENABLED, &flag, binsize)) {
  ...
  if (SysReg.GetDWORDValue(HKEY_CURRENT_USER, CFG_REG_KEY,
                           CFG_SOUND_SAMPLE_RATE, dword)) {
  ...
  if (SysReg.GetDWORDValue(HKEY_CURRENT_USER, CFG_REG_KEY, CFG_SOUND_VOLUME,
                           dword)) {
  ...
  RegRes =
      SysReg.GetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "RelaySoundEnabled",
                            &RelaySoundEnabled, binsize);
  if (!RegRes) {
    RelaySoundEnabled = 0;
  }

  RegRes =
      SysReg.GetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "TapeSoundEnabled",
                            &TapeSoundEnabled, binsize);
  if (!RegRes) {
    TapeSoundEnabled = 0; // BBC Tapes always were a bit harsh on the ears....
  }

  RegRes =
      SysReg.GetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "UsePrimaryBuffer",
                            &UsePrimaryBuffer, binsize);
  if (!RegRes) {
    UsePrimaryBuffer = 0;
  }

  RegRes = SysReg.GetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "Part Samples",
                                 &PartSamples, binsize);
  if (!RegRes) {
    PartSamples = 1;
  }
  ...
  RegRes =
      SysReg.GetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "ExponentialVolume",
                            &SoundExponentialVolume, binsize);
  if (!RegRes) {
    SoundExponentialVolume = 1;
  }
  RegRes = SysReg.GetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY,
                                 "TextToSpeechEnabled", &m_TextToSpeechEnabled,
                                 binsize);
  if (!RegRes) {
    m_TextToSpeechEnabled = 0;
  }

  if (SysReg.GetDWORDValue(HKEY_CURRENT_USER, CFG_REG_KEY, CFG_OPTIONS_STICKS,
                           dword)) {
  ...
  if (SysReg.GetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY,
                            CFG_OPTIONS_FREEZEINACTIVE, &flag, binsize)) {
    m_FreezeWhenInactive = flag;
  } else {
    GetPrivateProfileString(CFG_OPTIONS_SECTION, CFG_OPTIONS_FREEZEINACTIVE,
                            "1", CfgValue, sizeof(CfgValue), CFG_FILE_NAME);
  ...
  if (SysReg.GetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY,
                            CFG_OPTIONS_HIDE_CURSOR, &flag, binsize)) {
    m_HideCursor = flag;
  } else {
  ...
  if (SysReg.GetDWORDValue(HKEY_CURRENT_USER, CFG_REG_KEY,
                           CFG_OPTIONS_KEY_MAPPING, dword)) {
    m_MenuIdKeyMapping = dword;
  } else {
    sprintf(DefValue, "%d", IDM_DEFAULTKYBDMAPPING);
    GetPrivateProfileString(CFG_OPTIONS_SECTION, CFG_OPTIONS_KEY_MAPPING,
                            DefValue, CfgValue, sizeof(CfgValue),
                            CFG_FILE_NAME);
  ...
  RegRes = SysReg.GetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "KeyMapAS",
                                 &flag, binsize);
  if (!RegRes) {
    m_KeyMapAS = 0;
  } else {
  ...
  *binsize = 1;
  if (SysReg.GetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, CFG_AMX_ENABLED,
                            &flag, binsize)) {
    AMXMouseEnabled = flag;
  } else {
    GetPrivateProfileString(CFG_AMX_SECTION, CFG_AMX_ENABLED, "0", CfgValue,
                            sizeof(CfgValue), CFG_FILE_NAME);
  ...
  if (SysReg.GetDWORDValue(HKEY_CURRENT_USER, CFG_REG_KEY, CFG_AMX_LRFORMIDDLE,
                           dword)) {
    AMXLRForMiddle = dword;
  } else {
    GetPrivateProfileString(CFG_AMX_SECTION, CFG_AMX_LRFORMIDDLE, "1", CfgValue,
                            sizeof(CfgValue), CFG_FILE_NAME);
  ...
  if (SysReg.GetDWORDValue(HKEY_CURRENT_USER, CFG_REG_KEY, CFG_AMX_SIZE,
                           dword)) {
    m_MenuIdAMXSize = dword;
  } else {
    sprintf(DefValue, "%d", IDM_AMX_320X256);
    GetPrivateProfileString(CFG_AMX_SECTION, CFG_AMX_SIZE, DefValue, CfgValue,
                            sizeof(CfgValue), CFG_FILE_NAME);
  ...
  if (SysReg.GetDWORDValue(HKEY_CURRENT_USER, CFG_REG_KEY, CFG_AMX_ADJUST,
                           dword)) {
    m_MenuIdAMXAdjust = dword;
  } else {
    sprintf(DefValue, "%d", IDM_AMX_ADJUSTP30);
    GetPrivateProfileString(CFG_AMX_SECTION, CFG_AMX_ADJUST, DefValue, CfgValue,
                            sizeof(CfgValue), CFG_FILE_NAME);
  ...
  if (SysReg.GetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, CFG_PRINTER_ENABLED,
                            &flag, binsize)) {
    PrinterEnabled = flag;
  } else {
  ...
  if (SysReg.GetDWORDValue(HKEY_CURRENT_USER, CFG_REG_KEY, CFG_PRINTER_PORT,
                           dword)) {
  ...
  if (!SysReg.GetStringValue(HKEY_CURRENT_USER, CFG_REG_KEY, CFG_PRINTER_FILE,
                             m_PrinterFileName)) {
    GetPrivateProfileString(CFG_PRINTER_SECTION, CFG_PRINTER_FILE, "", CfgValue,
                            sizeof(CfgValue), CFG_FILE_NAME);
  ...
  //--	}
  //++
  if (SysReg.GetDWORDValue(HKEY_CURRENT_USER, CFG_REG_KEY, "Tape Clock Speed",
                           dword))
  ...
  *binsize = 1;
  RegRes = SysReg.GetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "UnlockTape",
                                 &flag, binsize);
  if (!RegRes) {
    UnlockTape = 0;
  } else {
  ...
  RegRes =
      SysReg.GetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "SerialPortEnabled",
                            &SerialPortEnabled, binsize);
  if (!RegRes) {
    SerialPortEnabled = 0;
  }

  RegRes =
      SysReg.GetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY,
                            "TouchScreenEnabled", &TouchScreenEnabled, binsize);
  if (!RegRes) {
    TouchScreenEnabled = 0;
  }

  RegRes = SysReg.GetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "SerialPort",
                                 &SerialPort, binsize);
  if (!RegRes) {
    SerialPort = 2;
  }
...
#ifdef WITH_ECONET
  RegRes = SysReg.GetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY,
                                 "EconetEnabled", &EconetEnabled, binsize);
  if (!RegRes) {
    EconetEnabled = 0;
  }
#endif
  ...
  RegRes = SysReg.GetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "TorchTube",
                                 &TorchTube, binsize);
  if (!RegRes) {
    TorchTube = 0;
  }

  RegRes = SysReg.GetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "AcornZ80",
                                 &AcornZ80, binsize);
  if (!RegRes) {
    AcornZ80 = 0;
  }

  RegRes = SysReg.GetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "TubeEnabled",
                                 &TubeEnabled, binsize);
  if (!RegRes) {
    TubeEnabled = 0;
  }
  ...
  RegRes = SysReg.GetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY,
                                 "Tube186Enabled", &Tube186Enabled, binsize);
  if (!RegRes) {
    Tube186Enabled = 0;
  }

  RegRes = SysReg.GetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "OpCodes",
                                 &OpCodes, binsize);
  ...
  RegRes = SysReg.GetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY,
                                 "Basic Hardware", &BHardware, binsize);
  if (!RegRes) {
    BHardware = 0;
  }

  RegRes = SysReg.GetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY,
                                 "Teletext Half Mode", &THalfMode, binsize);
  if (!RegRes) {
    THalfMode = 0;
  }

  RegRes = SysReg.GetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY,
                                 "SoundBlockSize", &SBSize, binsize);
  if (!RegRes) {
    SBSize = 0;
  }
  ...
  RegRes = SysReg.GetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY,
                                 "TeleTextAdapterEnabled",
                                 &TeleTextAdapterEnabled, binsize);
  if (!RegRes) {
    TeleTextAdapterEnabled = 0;
  }
  RegRes =
      SysReg.GetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "HardDriveEnabled",
                            &HardDriveEnabled, binsize);
  if (!RegRes) {
    HardDriveEnabled = 0;
  }
  ...
  if (SysReg.GetDWORDValue(HKEY_CURRENT_USER, CFG_REG_KEY, "CaptureResolution",
                           dword)) {
  ...
  if (SysReg.GetDWORDValue(HKEY_CURRENT_USER, CFG_REG_KEY, "FrameSkip",
                           dword)) {
  ...
  if (SysReg.GetDWORDValue(HKEY_CURRENT_USER, CFG_REG_KEY, CFG_SLEEP_TYPE,
                           dword))
    cfg_WaitType = dword;
  else
...
void BeebWin::SavePreferences() {
  ...
  *binsize = 1;
  SysReg.SetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "MachineType",
                        &MachineType, binsize);

  flag = m_DirectDrawEnabled;
  SysReg.SetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, CFG_VIEW_DIRECT_ENABLED,
                        &flag, binsize);
  flag = m_DDS2InVideoRAM;
  SysReg.SetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY,
                        CFG_VIEW_BUFFER_IN_VIDEO, &flag, binsize);
  SysReg.SetDWORDValue(HKEY_CURRENT_USER, CFG_REG_KEY, "DDFullScreenMode",
                       m_DDFullScreenMode);
  flag = m_isDD32;
  SysReg.SetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "isDD32", &flag,
                        binsize);
  SysReg.SetDWORDValue(HKEY_CURRENT_USER, CFG_REG_KEY, CFG_VIEW_WIN_SIZE,
                       m_MenuIdWinSize);
  flag = m_isFullScreen;
  SysReg.SetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "isFullScreen", &flag,
                        binsize);
  flag = m_ShowSpeedAndFPS;
  SysReg.SetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, CFG_VIEW_SHOW_FPS,
                        &flag, binsize);
  flag = palette_type;
  SysReg.SetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, CFG_VIEW_MONITOR, &flag,
                        binsize);
  SysReg.SetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "HideMenuEnabled",
                        &HideMenuEnabled, binsize);
  LEDByte = (DiscLedColour << 2) | ((LEDs.ShowDisc ? 1 : 0) << 1) |
            (LEDs.ShowKB ? 1 : 0);
  SysReg.SetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "LED Information",
                        &LEDByte, binsize);
  flag = m_MotionBlur;
  SysReg.SetDWORDValue(HKEY_CURRENT_USER, CFG_REG_KEY, "MotionBlur",
                       m_MotionBlur);
  *binsize = 8;
  SysReg.SetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "MotionBlurIntensities",
                        m_BlurIntensities, binsize);
  *binsize = 1;
  SysReg.SetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "TextViewEnabled",
                        &m_TextViewEnabled, binsize);

  SysReg.SetDWORDValue(HKEY_CURRENT_USER, CFG_REG_KEY, CFG_SPEED_TIMING,
                       m_MenuIdTiming);

  SysReg.SetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, CFG_SOUND_ENABLED,
                        &SoundDefault, binsize);
  flag = SoundChipEnabled;
  SysReg.SetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "SoundChipEnabled",
                        &flag, binsize);
  flag = DirectSoundEnabled;
  SysReg.SetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY,
                        CFG_SOUND_DIRECT_ENABLED, &flag, binsize);
  SysReg.SetDWORDValue(HKEY_CURRENT_USER, CFG_REG_KEY, CFG_SOUND_SAMPLE_RATE,
                       m_MenuIdSampleRate);
  SysReg.SetDWORDValue(HKEY_CURRENT_USER, CFG_REG_KEY, CFG_SOUND_VOLUME,
                       m_MenuIdVolume);
  SysReg.SetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "RelaySoundEnabled",
                        &RelaySoundEnabled, binsize);
  SysReg.SetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "TapeSoundEnabled",
                        &TapeSoundEnabled, binsize);
  SysReg.SetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "UsePrimaryBuffer",
                        &UsePrimaryBuffer, binsize);
  SysReg.SetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "Part Samples",
                        &PartSamples, binsize);
  //	SysReg.SetBinaryValue(HKEY_CURRENT_USER,CFG_REG_KEY,"UseHostClock",&UseHostClock,binsize);
  SysReg.SetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "ExponentialVolume",
                        &SoundExponentialVolume, binsize);
  SysReg.SetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "TextToSpeechEnabled",
                        &m_TextToSpeechEnabled, binsize);

  SysReg.SetDWORDValue(HKEY_CURRENT_USER, CFG_REG_KEY, CFG_OPTIONS_STICKS,
                       m_MenuIdSticks);
  flag = m_FreezeWhenInactive;
  SysReg.SetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY,
                        CFG_OPTIONS_FREEZEINACTIVE, &flag, binsize);
  flag = m_HideCursor;
  SysReg.SetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, CFG_OPTIONS_HIDE_CURSOR,
                        &flag, binsize);
  SysReg.SetDWORDValue(HKEY_CURRENT_USER, CFG_REG_KEY, CFG_OPTIONS_KEY_MAPPING,
                       m_MenuIdKeyMapping);
  flag = m_KeyMapAS;
  SysReg.SetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "KeyMapAS", &flag,
                        binsize);
  flag = m_KeyMapFunc;
  SysReg.SetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "KeyMapFunc", &flag,
                        binsize);

  SysReg.SetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "DisableKeysBreak",
                        &m_DisableKeysBreak, binsize);
  SysReg.SetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "DisableKeysEscape",
                        &m_DisableKeysEscape, binsize);
  SysReg.SetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "DisableKeysShortcut",
                        &m_DisableKeysShortcut, binsize);
  ...
  *binsize = 1;
  ...
  SysReg.SetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, CFG_AMX_ENABLED, &flag,
                        binsize);
  SysReg.SetDWORDValue(HKEY_CURRENT_USER, CFG_REG_KEY, CFG_AMX_LRFORMIDDLE,
                       AMXLRForMiddle);
  SysReg.SetDWORDValue(HKEY_CURRENT_USER, CFG_REG_KEY, CFG_AMX_SIZE,
                       m_MenuIdAMXSize);
  SysReg.SetDWORDValue(HKEY_CURRENT_USER, CFG_REG_KEY, CFG_AMX_ADJUST,
                       m_MenuIdAMXAdjust);

  flag = PrinterEnabled;
  SysReg.SetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, CFG_PRINTER_ENABLED,
                        &flag, binsize);
  SysReg.SetDWORDValue(HKEY_CURRENT_USER, CFG_REG_KEY, CFG_PRINTER_PORT,
                       m_MenuIdPrinterPort);
  SysReg.SetStringValue(HKEY_CURRENT_USER, CFG_REG_KEY, CFG_PRINTER_FILE,
                        m_PrinterFileName);
  ...
  *binsize = 1;
  flag = UnlockTape;
  SysReg.SetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "UnlockTape", &flag,
                        binsize);

  SysReg.SetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "TouchScreenEnabled",
                        &TouchScreenEnabled, binsize);

  SysReg.SetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "SerialPortEnabled",
                        &SerialPortEnabled, binsize);
  SysReg.SetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "SerialPort",
                        &SerialPort, binsize);

#ifdef WITH_ECONET
  SysReg.SetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "EconetEnabled",
                        &EconetEnabled, binsize); // Rob
#endif

  flag = SpeechDefault;
  SysReg.SetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "SpeechEnabled", &flag,
                        binsize);

  SysReg.SetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "TorchTube", &TorchTube,
                        binsize);
  SysReg.SetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "AcornZ80", &AcornZ80,
                        binsize);
  SysReg.SetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "Tube186Enabled",
                        &Tube186Enabled, binsize);
  SysReg.SetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "TubeEnabled",
                        &TubeEnabled, binsize);
  SysReg.SetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "OpCodes", &OpCodes,
                        binsize);
  SysReg.SetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "Basic Hardware",
                        &BHardware, binsize);
  SysReg.SetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "Teletext Half Mode",
                        &THalfMode, binsize);
  SysReg.SetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "SoundBlockSize",
                        &SBSize, binsize);
  SysReg.SetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY,
                        "TeleTextAdapterEnabled", &TeleTextAdapterEnabled,
                        binsize);
  SysReg.SetBinaryValue(HKEY_CURRENT_USER, CFG_REG_KEY, "HardDriveEnabled",
                        &HardDriveEnabled, binsize);

  SysReg.SetDWORDValue(HKEY_CURRENT_USER, CFG_REG_KEY, "CaptureResolution",
                       m_MenuIdAviResolution);
  SysReg.SetDWORDValue(HKEY_CURRENT_USER, CFG_REG_KEY, "FrameSkip",
                       m_MenuIdAviSkip);

  //+>
  // Additional BeebEm SDL options:

  SysReg.SetDWORDValue(HKEY_CURRENT_USER, CFG_REG_KEY, CFG_EMULATECRTGRAPHICS,
                       cfg_EmulateCrtGraphics);
  SysReg.SetDWORDValue(HKEY_CURRENT_USER, CFG_REG_KEY, CFG_EMUALTECRTTELETEXT,
                       cfg_EmulateCrtTeletext);
  SysReg.SetDWORDValue(HKEY_CURRENT_USER, CFG_REG_KEY, CFG_WANTLOWLATENCYSOUND,
                       cfg_WantLowLatencySound);
  SysReg.SetDWORDValue(HKEY_CURRENT_USER, CFG_REG_KEY, CFG_SLEEP_TYPE,
                       cfg_WaitType);

  SysReg.SetDWORDValue(HKEY_CURRENT_USER, CFG_REG_KEY, CFG_WINDOWEDRESOLUTION,
                       cfg_Windowed_Resolution);
  SysReg.SetDWORDValue(HKEY_CURRENT_USER, CFG_REG_KEY, CFG_FULLSCREENRESOLUTION,
                       cfg_Fullscreen_Resolution);
...
void BeebWin::SetTapeSpeedMenu(void) {
  CheckMenuItem(m_hMenu, ID_TAPE_FAST,
                (TapeClockSpeed == 750) ? MF_CHECKED : MF_UNCHECKED);
  CheckMenuItem(m_hMenu, ID_TAPE_MFAST,
                (TapeClockSpeed == 1600) ? MF_CHECKED : MF_UNCHECKED);
  CheckMenuItem(m_hMenu, ID_TAPE_MSLOW,
                (TapeClockSpeed == 3200) ? MF_CHECKED : MF_UNCHECKED);
  CheckMenuItem(m_hMenu, ID_TAPE_NORMAL,
                (TapeClockSpeed == 5600) ? MF_CHECKED : MF_UNCHECKED);
}
...
void BeebWin::UpdateLEDMenu(HMENU hMenu) {
  // Update the LED Menu
  CheckMenuItem(hMenu, ID_RED_LEDS,
                (DiscLedColour > 0) ? MF_UNCHECKED : MF_CHECKED);
  CheckMenuItem(hMenu, ID_GREEN_LEDS,
                (DiscLedColour > 0) ? MF_CHECKED : MF_UNCHECKED);
  CheckMenuItem(hMenu, ID_SHOW_KBLEDS,
                (LEDs.ShowKB) ? MF_CHECKED : MF_UNCHECKED);
  CheckMenuItem(hMenu, ID_SHOW_DISCLEDS,
                (LEDs.ShowDisc) ? MF_CHECKED : MF_UNCHECKED);
}

void BeebWin::UpdateOptiMenu(void) {
  CheckMenuItem(m_hMenu, ID_DOCONLY,
                (OpCodes == 1) ? MF_CHECKED : MF_UNCHECKED);
  CheckMenuItem(m_hMenu, ID_EXTRAS, (OpCodes == 2) ? MF_CHECKED : MF_UNCHECKED);
  CheckMenuItem(m_hMenu, ID_FULLSET,
                (OpCodes == 3) ? MF_CHECKED : MF_UNCHECKED);
  CheckMenuItem(m_hMenu, ID_BHARDWARE,
                (BHardware == 1) ? MF_CHECKED : MF_UNCHECKED);
  CheckMenuItem(m_hMenu, ID_TSTYLE,
                (THalfMode == 1) ? MF_CHECKED : MF_UNCHECKED);
...
void BeebWin::HandleCommand(int MenuId) {
  ...
  case ID_ECONET:
    ...
    //--			}
    //--			else
    //--				CheckMenuItem(hMenu, IDM_DD32ONOFF, m_isDD32 ?
    //MF_CHECKED
    //: MF_UNCHECKED);
    //--		}
    //--		break;
    ...
    //--			HandleCommand(IDM_FULLSCREEN);
    //--			if (!m_DirectDrawEnabled)
    //--				// Should not happen since the items are grayed
    //out, but anyway...
  ...
  case IDM_ALLOWWRITES_ROMF:
    ...
    CheckMenuItem(hMenu, MenuId,
                  RomWritable[(MenuId - IDM_ALLOWWRITES_ROM0)] ? MF_UNCHECKED
                                                               : MF_CHECKED);
  ...
  case IDM_TUBE:
    ...
    CheckMenuItem(hMenu, IDM_TUBE186,
                  (Tube186Enabled) ? MF_CHECKED : MF_UNCHECKED);
  ...
  case ID_UNLOCKTAPE:
    ...
    CheckMenuItem(hMenu, ID_UNLOCKTAPE,
                  (UnlockTape) ? MF_CHECKED : MF_UNCHECKED);
    break;
  ...
  case IDM_TEXTVIEW:
    ...
    //--		{
    //--			// Give user warning
    //--			if (MessageBox(m_hWnd,"Disabling the Windows keys will
    //affect the whole PC.\n"
    ...
    //--			// SysReg.DeleteKey(HKEY_LOCAL_MACHINE,
    //CFG_KEYBOARD_LAYOUT
    //"\\" CFG_SCANCODE_MAP);
    //--			*binsize=0;
    ...
    //--		{
    //--			// Ask user for reboot
    //--			if (MessageBox(m_hWnd,"Reboot required for key
...
void BeebWin::SetPBuff(void) {
  CheckMenuItem(m_hMenu, ID_PBUFF,
                (UsePrimaryBuffer) ? MF_CHECKED : MF_UNCHECKED);
}

void BeebWin::SetSoundMenu(void) {
  CheckMenuItem(m_hMenu, IDM_SOUNDONOFF,
                (SoundEnabled) ? MF_CHECKED : MF_UNCHECKED);
  SetPBuff();
}
...
BOOL BeebWin::IsFrozen(void) { return m_frozen; }
...
void SaveEmuUEF(FILE *SUEF) {
  ...
  fput32(16, SUEF);
  // Emulator Specifics
  // Note about this block: It should only be handled by beebem from
  // uefstate.cpp if the UEF has been determined to be from BeebEm (Block 046C)
...
void BeebWin::LoadFDC(const char *DLLName, bool save) {
  ...
  if (DLLName == NULL) {
    RegRes =
        SysReg.GetStringValue(HKEY_CURRENT_USER, CFG_REG_KEY, CfgName, FDCDLL);
    if (!RegRes) {
      strcpy(FDCDLL, "None");
      if (MachineType == 2) {
  ...
  if (strcmp(DLLName, "None")) {
    ...
    //--			if ((PGetBoardProperties==NULL) ||
    //(PSetDriveControl==NULL)
    //|| (PGetDriveControl==NULL)) {
    //--				MessageBox(GETHWND,"Invalid FDD Extension
    //Board DLL\nReverting to native 8271\n",WindowTitle,MB_OK|MB_ICONERROR);
    //--				strcpy(DLLName, "None");
    //--			}
    //--			else {
...
void BeebWin::SetDriveControl(unsigned char value) {
  // This takes a value from the mem/io decoder, as written by the cpu, runs it
  // through the DLL's translator, then sends it on to the 1770 FDC in master
  // 128 form.
...
void BeebWin::InitTextToSpeech(void) {
  ...
  //--			CLSID_SpVoice, NULL, CLSCTX_ALL, IID_ISpVoice, (void
  //**)&m_SpVoice);
  //--		if( FAILED( hr ) )
  //--		{
  //--			m_SpVoice = NULL;
...
void BeebWin::TextToSpeechReadWord(void) {
  //--	char text[MAX_SPEECH_LINE_LEN+1];
  //--	int i = 0;
  //--	for (int c = m_SpeechCol; m_SpeechText[c] != 0 && m_SpeechText[c] != '
  //'; ++c)
  //--	{
...
void BeebWin::TextToSpeechReadScreen(void) {
  ...
  //--	int i = 0;
  //--
  //--	for (m_SpeechLine = 0; m_SpeechLine < CRTC_VerticalDisplayed-1;
  //++m_SpeechLine)
  //--	{
  //--		VideoGetText(m_SpeechText, m_SpeechLine);
...
//--		bool insPressed = (GetKeyState(VK_INSERT) < 0) ||
//(GetKeyState(VK_NUMPAD0) < 0);
//--
...
void BeebWin::TextView(void) {
  ...
  //--		SendMessage(m_hTextView, WM_SETTEXT, 0,
  //(LPARAM)m_TextViewScreen);
