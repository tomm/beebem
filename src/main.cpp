...
void ShowingMenu(void) { showing_menu = 1; }

void NoMenuShown(void) { showing_menu = 0; }
...
int main(int argc, char *argv[]) {
  ...
  */
   ...
   */
  X11_CapsLock_Down = 0; // =0 not down, used to emulate a key release in X11
                         // (caps has no release event).
  ...
  do {
    ...
    /* As X11 sucks, we need to release the Caps Lock key ourselves, an event
     * will not happen. The X11_CapsLock_Down var basically counts down once per
     * pass of the emulator core, when it's 1, we assume enough time has passed
     * to pass the core a release of the Beebs Caps Lock key.
     */
    ...
    /* Toggle processing of either events to the emulator core, or events to the
     * menu.
     */
    SDL_Event event;
      ...
      while (SDL_PollEvent(&event))
        ...
        case SDL_MOUSEMOTION:
          ...
          if (mainWin) {
            ...
            // Experiment: show menu in full screen when cursor moved to top of
            // window
            //-- if (HideMenuEnabled)
            //-- {
            //-- 	if (HIWORD(lParam) <= 2)
