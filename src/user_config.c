...
static BOOL CopyFile(char *source_file, char *dest_file) {
   ...
   * be a security risk).
   */
  if (source_file == NULL || dest_file == NULL || strlen(source_file) == 0 ||
      strlen(dest_file) == 0 || source_file[0] != '/' || dest_file[0] != '/') {
...
static BOOL CreateDirectory(char *file) {
  ...
  errno = 0;
  if (mkdir(file, (mode_t)S_IRUSR | S_IWUSR | S_IXUSR | S_IRGRP | S_IXGRP |
                      S_IROTH | S_IXOTH) == 0)
    return TRUE;
...
char *GetUserConfigPath(char *path, size_t len) {
  ...
  if (!TestDirectory(path)) {
    ...
    //#	endif
    EG_Draw_FlushEventQueue();
    EG_MessageBox(screen_ptr, EG_MESSAGEBOX_INFORMATION, "Welcome to BeebEm!",
                  WELCOME_MESSAGE, "OK", NULL, NULL, NULL, 0);
...
#define NUMBER_OF_FILES 9
  char files[NUMBER_OF_FILES][20] = {"sasi0.dat", "scsi0.dat", "scsi0.dsc",
                                     "scsi1.dat", "scsi1.dsc", "scsi2.dat",
                                     "scsi2.dsc", "scsi3.dat", "scsi3.dsc"};
