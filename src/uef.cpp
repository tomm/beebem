...
void uef_setunlock(int unlock) { uef_unlock = unlock; }
    ...
    if (ch->type >= 0x100 && ch->type <= 0x1ff) {
      if (ch->len > 0) {
        ...
        if (ch->data == NULL) {
          ...
          uef_chunks++;
          ch = (uef_chunk_info *)realloc(uef_chunk, (uef_chunks + 1) *
                                                        sizeof(uef_chunk_info));
          if (ch == NULL) {
            uef_errno = UEF_OPEN_MEMERR;
            error = 1;
          } else {
...
int uef_getdata(int time) {
  ...
  if (uef_last_chunk != NULL && time >= uef_last_chunk->start_time &&
      time < uef_last_chunk->end_time) {
    ch = uef_last_chunk;
    found = 1;
  } else {
  ...
  case 0x104: /* Data block */
    ...
    i = (int)((double)(time - ch->start_time) /
              (ch->end_time - ch->start_time) * (ch->len - j));
  ...
  case 0x111: /* HTone with dummy byte */
    i = (int)((double)(time - ch->start_time) /
              (ch->end_time - ch->start_time) * (ch->l1 + 160 + ch->l2));
  ...
  case 0x114: /* Security waves */
    i = (int)((double)(time - ch->start_time) /
              (ch->end_time - ch->start_time) * ch->l1);
...
int uef_putdata(int data, int time) {
  ...
  if (UEFRES_TYPE(data) == UEF_HTONE || UEFRES_TYPE(data) == UEF_GAP) {
    ...
    if (uef_put_chunk.data == NULL)
      ...
      datap =
          (unsigned char *)realloc(uef_put_chunk.data, uef_put_chunk.len + 1);
...
static int uef_write_chunk(void) {
  ...
  if (ok) {
    ...
    case 0x110: /* HTone */
      gzput32(uef_file, 2);
      /* Assume 1200 baud */
      l = (int)((double)(uef_put_chunk.end_time - uef_put_chunk.start_time) *
                (1200 * 2.0) / NORMAL_TAPE_SPEED);
      gzput16(uef_file, l);
      break;
    case 0x112: /* Gap */
      gzput32(uef_file, 2);
