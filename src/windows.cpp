...
int MessageBox(HWND hwnd, const char *message_p, const char *title_p,
  ...
  case MB_OKCANCEL:
    if (EG_MessageBox(screen_ptr, icon_type, title_p, message_p, "OK", "CANCEL",
                      NULL, NULL, selected) == 1)
      ret = IDOK;
    else
      ret = IDCANCEL;
    break;
  case MB_ABORTRETRYIGNORE:
    ret = EG_MessageBox(screen_ptr, icon_type, title_p, message_p, "Abort",
                        "Retry", "Ignore", NULL, selected);
    switch (ret) {
    case 2:
      ret = IDRETRY;
      break;
    ...
    break;
  case MB_YESNOCANCEL:
    ret = EG_MessageBox(screen_ptr, icon_type, title_p, message_p, "Yes", "No",
                        "Cancel", NULL, selected);
    switch (ret) {
    case 2:
      ret = IDNO;
      break;
    ...
    break;
  case MB_YESNO:
    if (EG_MessageBox(screen_ptr, icon_type, title_p, message_p, "Yes", "No",
                      NULL, NULL, selected) == 1)
      ret = IDYES;
    else
      ret = IDNO;
    break;
  case MB_RETRYCANCEL:
    if (EG_MessageBox(screen_ptr, icon_type, title_p, message_p, "Retry",
                      "Cancel", NULL, NULL, selected) == 1)
      ret = IDRETRY;
    else
      ret = IDCANCEL;
    break;
  ...
  case MB_OK:
  default:
    (void)EG_MessageBox(screen_ptr, icon_type, title_p, message_p, "OK", NULL,
                        NULL, NULL, 1);
    ret = IDOK;
    break;
  }
...
void Sleep(DWORD ticks) { SaferSleep((unsigned int)ticks); }

DWORD GetTickCount(void) { return (SDL_GetTicks()); }
...
UINT GetMenuState(HMENU hMenu, UINT uId, UINT uFlags) {
  ...
  contains the menu flags associated with the item, and the high-order byte
  contains the number of items in the submenu opened by the item.
  ...
  Remarks
      ...
      Note  The GetMenuState function has been superseded by the
  GetMenuItemInfo. You can still use GetMenuState, however, if you do not need
  any of the extended features of GetMenuItemInfo.

      In addition, it is possible to test an item for a flag value of
  MF_ENABLED, MF_STRING, MF_UNCHECKED, or MF_UNHILITE. However, since these
  values equate to zero you must use an expression to test for them. Flag
  Expression to test for the flag MF_ENABLED	! (Flag&(MF_DISABLED |
  MF_GRAYED)) MF_STRING	! (Flag&(MF_BITMAP | MF_OWNERDRAW))
...
DWORD GetPrivateProfileString(LPCTSTR lpAppName, LPCTSTR lpKeyName,
  ...
  lpDefault
      [in] A default string. If the lpKeyName key cannot be found in the
  initialization file, GetPrivateProfileString copies the default string to the
  ...
  trailing blanks.
          ...
          Windows Me/98/95:  Although lpDefault is declared as a constant
  parameter, the system strips any trailing blanks by inserting a null character
  ...
  */
  ...
  ret = GetFakeRegistryItem_String((char *)lpKeyName,
                                   (char **)&fake_reg_string_ptr);
  if (ret == TRUE) {
