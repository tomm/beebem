...
// [TODO] MenuItemCheck function etc (based on ID only, searches for widget in
// above array and then changes it's state based on the widget type).
...
int GetGUIOption(int windows_menu_id) {
  ...
  for (i = 0; i < win_menu_count; i++) {
    if (win_menu[i].windows_menu_id == windows_menu_id) {
      ...
      //			if (EG_Widget_GetType(ptr) == ((EG_StringHash)
      // EG_Widget_Type_TickBox) )
      // return(EG_TickBox_IsTicked(ptr));
    }
  }
  return -1;
...
int UpdateGUIOption(int windows_menu_id, int is_selected) {
  ...
  for (i = 0; i < win_menu_count; i++) {
    if (win_menu[i].windows_menu_id == windows_menu_id) {
      ...
      // printf("Found: %s is of type %X %X %X\n", EG_Widget_GetName(ptr),
      // (EG_StringHash) EG_Widget_GetType(ptr) , (EG_StringHash)
      // EG_Widget_Type_TickBox, EG_Widget_Type_RadioButton);
      ...
      if (EG_Widget_GetType(ptr) ==
          ((EG_StringHash)EG_Widget_Type_ToggleButton)) {
...
EG_BOOL InitializeBeebEmGUI(SDL_Surface *screen_ptr) {
  ...
  //              EG_Log(EG_LOG_FATAL, dL"Unable to create About Menu -
  //              exiting!", dR);
  //		return(EG_FALSE);
  //      }
...
//	if (strcmp(EG_Widget_GetName(widget_ptr), "machine_bbc_b") ==0 &&
// MachineType != 0){ 		force_reboot = EG_TRUE;
// printf("bbc_b\n");
//	}
//	if (strcmp(EG_Widget_GetName(widget_ptr), "machine_integra_b") ==0 &&
// MachineType != 1){ 		force_reboot = EG_TRUE;
// printf("integra_b\n");
//	}
//	if (strcmp(EG_Widget_GetName(widget_ptr), "machine_bbc_b_plus") == 0 &&
// MachineType != 2){ 		force_reboot = EG_TRUE;
// printf("b_plus\n");
//	}
//	if (strcmp(EG_Widget_GetName(widget_ptr), "machine_bbc_master_128") == 0
//&& MachineType != 3){ 		force_reboot = EG_TRUE;
// printf("master_128\n");
//	}
//
//	// Yes, force reboot.
//	if (force_reboot == EG_TRUE){
//		if (EG_RadioButton_IsSelected(gui->widget_reset_ptr) !=
// EG_TRUE){ 			EG_RadioGroup_Select(gui->widget_reset_ptr);
...
static void Main_Button_Speed(ATTR_UNUSED EG_Widget *widget_ptr,
                              ATTR_UNUSED void *user_ptr) {
  EG_Window_Hide(gui.win_menu_ptr);
  SDL_Delay(100);
...
static void Main_Button_About(ATTR_UNUSED EG_Widget *widget_ptr,
                              ATTR_UNUSED void *user_ptr) {
  EG_Window_Hide(gui.win_menu_ptr);
  SDL_Delay(100);
  ...
  //	EG_TextBox(screen_ptr,"BeebEm", ABOUT_MESSAGE, "Back", NULL, NULL, NULL,
  // 1);

  EG_Draw_FlushEventQueue();
  // printf("MESSAGE BOX\n");
  EG_MessageBox(screen_ptr, EG_MESSAGEBOX_INFORMATION, "BeebEm for UNIX!",
                ABOUT_MESSAGE, "OK", NULL, NULL, NULL, 0);

  EG_Window_Show(gui.win_menu_ptr);
}

static void Main_Button_Devices(ATTR_UNUSED EG_Widget *widget_ptr,
                                ATTR_UNUSED void *user_ptr) {
  EG_Window_Hide(gui.win_menu_ptr);
  SDL_Delay(100);
...
static void Main_Button_Disks(ATTR_UNUSED EG_Widget *widget_ptr,
                              ATTR_UNUSED void *user_ptr) {
  EG_Window_Hide(gui.win_menu_ptr);
  SDL_Delay(100);
...
static void Main_Button_Tapes(ATTR_UNUSED EG_Widget *widget_ptr,
                              ATTR_UNUSED void *user_ptr) {
  EG_Window_Hide(gui.win_menu_ptr);
  SDL_Delay(100);
...
static void Main_Button_Keyboard(ATTR_UNUSED EG_Widget *widget_ptr,
                                 ATTR_UNUSED void *user_ptr) {
  EG_Window_Hide(gui.win_menu_ptr);
  SDL_Delay(100);
...
static void Main_Button_AMX(ATTR_UNUSED EG_Widget *widget_ptr,
                            ATTR_UNUSED void *user_ptr) {
  EG_Window_Hide(gui.win_menu_ptr);
  SDL_Delay(100);
...
static EG_BOOL Make_Main(SDL_Surface *dst_ptr) {
  ...
  //	widget_ptr =  EG_Button_Create("main_oka222222y", col,
  // EG_BUTTON_ALIGN_CENTER, "Okay", loc);

  /* Okay button
   */
  loc = CalcRect(win.w - 110, win.h - 10 - 17 - 17 - 17, 100, 14 * 3);
  gui.widget_okay_ptr =
      EG_Button_Create("main_okay", col, EG_BUTTON_ALIGN_CENTER, "Okay", loc);
  (void)EG_Button_SetMyCallback_OnClick(gui.widget_okay_ptr, Main_Button_Okay,
                                        (void *)window_ptr);
  EG_Window_AddWidget(window_ptr, gui.widget_okay_ptr);

  //        loc = CalcRect(win.w-110, win.h-10-17-17-17, 100, 14*3);
  //        widget_ptr = EG_Button_Create("main_okay", col,
  //        EG_BUTTON_ALIGN_CENTER, "Okay", loc); (void)
  //        EG_Button_SetMyCallback_OnClick(widget_ptr, Main_Button_Okay,
  //        (void*) window_ptr); (void) EG_Window_AddWidget(window_ptr,
  //        widget_ptr);
  //
  ...
  //	gui.widget_reset_ptr = EG_RadioButton_Create("main_reset", col, "Reset
  // BBC", loc);
  //
  //	loc = CalcRect(10, win.h-10-17-17, 150, 16);
  //	gui.widget_no_reset_ptr = EG_RadioButton_Create("main_noreset", col, "No
  // Reset", loc);
  //
  ...
  (void)EG_TickBox_SetMyCallback_OnClick(widget_ptr, Main_Tick_FullScreen,
                                         &gui);
  //(void) EG_TickBox_Disable(widget_ptr);
  EG_Window_AddWidget(window_ptr, widget_ptr);

#define MENU_CALC_LOC(c, r, b)                                                 \
  loc = CalcRect(10 + c * (win.w / 2 - 30) + 10 + (14 * c),                    \
                 30 + (r + 1) * 20 + b * 10, win.w / 2 - 20 - 5, 19)

  /* Top title label
   */
  loc = CalcRect(0, 10, win.w, 20);
  col = TITLE_COLORS; // CalcColor(191+32, 64, 32);
  widget_ptr = EG_Label_Create("lab_title", col, EG_LABEL_ALIGN_CENTER,
                               "BeebEm UNIX (" VERSION ")", loc);
  ...
  widget_ptr = EG_Button_Create("but_system", col, EG_BUTTON_ALIGN_CENTER,
                                "System", loc);
  ...
  widget_ptr = EG_Button_Create("but_screen", col, EG_BUTTON_ALIGN_CENTER,
                                "Screen", loc);
  ...
  //        col = CalcColor(191, 191, 191);
  widget_ptr =
      EG_Button_Create("but_sound", col, EG_BUTTON_ALIGN_CENTER, "Sound", loc);
   ...
   */
  MENU_CALC_LOC(1, 1, 0);
  widget_ptr = EG_Button_Create("but_roms", col, EG_BUTTON_ALIGN_CENTER,
                                "ROM writes", loc);
  ...
  MENU_CALC_LOC(0, 2, 0);
  widget_ptr =
      EG_Button_Create("but_speed", col, EG_BUTTON_ALIGN_CENTER, "Speed", loc);
  ...
  MENU_CALC_LOC(1, 2, 0);
  widget_ptr =
      EG_Button_Create("but_about", col, EG_BUTTON_ALIGN_CENTER, "About", loc);
   ...
   */
  MENU_CALC_LOC(0, 3, 0);
  widget_ptr = EG_Button_Create("but_devices", col, EG_BUTTON_ALIGN_CENTER,
                                "Devices", loc);
  ...
  widget_ptr = EG_Button_Create("but_disks", col, EG_BUTTON_ALIGN_CENTER,
                                "Keyboard", loc);
  ...
  MENU_CALC_LOC(0, 5, 0);
  widget_ptr =
      EG_Button_Create("but_amx", col, EG_BUTTON_ALIGN_CENTER, "AMX", loc);
  ...
  MENU_CALC_LOC(0, 6, 0);
  widget_ptr =
      EG_Button_Create("but_tapes", col, EG_BUTTON_ALIGN_CENTER, "Tapes", loc);
  ...
  MENU_CALC_LOC(1, 5, 0);
  widget_ptr =
      EG_Button_Create("but_reset", col, EG_BUTTON_ALIGN_CENTER, "Reset", loc);
  ...
  MENU_CALC_LOC(1, 6, 0);
  widget_ptr =
      EG_Button_Create("but_quit", col, EG_BUTTON_ALIGN_CENTER, "Quit", loc);
...
static void Disks_AcornDFS(ATTR_UNUSED EG_Widget *widget_ptr,
                           ATTR_UNUSED void *user_ptr) {
  mainWin->LoadFDC("Acorn", true);
  //	EG_Label_SetCaption(gui.widget_fdc_label, "Acorn");
}

static void Disks_WatfordDFS(ATTR_UNUSED EG_Widget *widget_ptr,
                             ATTR_UNUSED void *user_ptr) {
  mainWin->LoadFDC("Watford", true);
  //	EG_Label_SetCaption(gui.widget_fdc_label, "Watford");
}

static void Disks_OpusDFS(ATTR_UNUSED EG_Widget *widget_ptr,
                          ATTR_UNUSED void *user_ptr) {
  mainWin->LoadFDC("Opus", true);
  //	EG_Label_SetCaption(gui.widget_fdc_label, "Opus");
}

static void Disks_NativeDFS(ATTR_UNUSED EG_Widget *widget_ptr,
                            ATTR_UNUSED void *user_ptr) {
...
void Update_FDC_Buttons(void) {
  ...
  RegRes =
      SysReg.GetStringValue(HKEY_CURRENT_USER, CFG_REG_KEY, CfgName, buffer);
  if (!RegRes) {
    strcpy(buffer, "None");
    if (MachineType == 2)
...
static void System_Button_Back(ATTR_UNUSED EG_Widget *widget_ptr,
                               ATTR_UNUSED void *user_ptr) {
...
static EG_BOOL Make_System(SDL_Surface *dst_ptr) {
  ...
  */
  ...
  /* Title label
   */
#define SYSTEM_CALC_LOC(r, b)                                                  \
  loc = CalcRect(20, 10 + r * 17 + b * 10, win.w - 40, 16)

  SYSTEM_CALC_LOC(0, 0);
  widget_ptr = EG_Label_Create("system:model:title", col, EG_LABEL_ALIGN_CENTER,
                               "BBC model:", loc);
  (void)EG_Window_AddWidget(window_ptr, widget_ptr);
  ...
  (void)EG_TickBox_SetMyCallback_OnClick(
      widget_ptr, System_Change_Model,
      CreateWindowsMenuItem(ID_MODELB, widget_ptr));
  gui.widget_machine_bbc_b = widget_ptr;

  SYSTEM_CALC_LOC(3, 0);
  widget_ptr =
      EG_RadioButton_Create("integra_b", col, "BBC Model B + Integra-B", loc);
  EG_RadioGroup_AddButton(group1_ptr, widget_ptr);
  (void)EG_TickBox_SetMyCallback_OnClick(
      widget_ptr, System_Change_Model,
  ...
  SYSTEM_CALC_LOC(4, 0);
  widget_ptr =
      EG_RadioButton_Create("bbc_b_plus", col, "BBC Model B Plus", loc);
  EG_RadioGroup_AddButton(group1_ptr, widget_ptr);
  (void)EG_TickBox_SetMyCallback_OnClick(
      widget_ptr, System_Change_Model,
  ...
  SYSTEM_CALC_LOC(5, 0);
  widget_ptr =
      EG_RadioButton_Create("bbc_master_128", col, "BBC Master 128", loc);
  EG_RadioGroup_AddButton(group1_ptr, widget_ptr);
  (void)EG_TickBox_SetMyCallback_OnClick(
      widget_ptr, System_Change_Model,
  ...
  SYSTEM_CALC_LOC(7, 0);
  widget_ptr = EG_Label_Create("system:6502:title", col, EG_LABEL_ALIGN_CENTER,
                               "Floppy Disc Controller:", loc);
  EG_Window_AddWidget(window_ptr, widget_ptr);
  ...
  (void)EG_ToggleButton_SetMyCallback_OnClick(widget_ptr, Disks_NativeDFS,
                                              NULL);
  ...
  //	(void) EG_Window_AddWidget(window_ptr, widget_ptr);
  (void)EG_RadioGroup_AddButton(group3_ptr, widget_ptr);
  (void)EG_ToggleButton_SetMyCallback_OnClick(widget_ptr, Disks_WatfordDFS,
                                              NULL);
  ...
  //      widget_ptr = EG_Label_Create("disks:fdc_label", col,
  //      EG_LABEL_ALIGN_CENTER, "", loc);
  //    (void) EG_Window_AddWidget(window_ptr, widget_ptr);
  ...
  widget_ptr = EG_RadioButton_Create("system:6502:documented", col,
                                     "Documented only", loc);
  EG_RadioGroup_AddButton(group2_ptr, widget_ptr);
  (void)EG_TickBox_SetMyCallback_OnClick(
      widget_ptr, ProcessGUIOption,
      CreateWindowsMenuItem(ID_DOCONLY, widget_ptr));

  SYSTEM_CALC_LOC(15, 0);
  widget_ptr =
      EG_RadioButton_Create("system:6502:extras", col, "Common extras", loc);
  EG_RadioGroup_AddButton(group2_ptr, widget_ptr);
  (void)EG_TickBox_SetMyCallback_OnClick(
      widget_ptr, ProcessGUIOption,
  ...
  widget_ptr =
      EG_RadioButton_Create("system:6502:fullset", col, "Full set", loc);
  EG_RadioGroup_AddButton(group2_ptr, widget_ptr);
  ...
  widget_ptr = EG_TickBox_Create("system:6502:ignor_illegal", col,
                                 "Ignore Illegal Instructions", loc);
  EG_Window_AddWidget(window_ptr, widget_ptr);
  (void)EG_TickBox_SetMyCallback_OnClick(
      widget_ptr, ProcessGUIOption,
  ...
  widget_ptr = EG_Button_Create("but_system_back", col, EG_BUTTON_ALIGN_CENTER,
                                "Back", loc);
...
static void Screen_Button_Back(ATTR_UNUSED EG_Widget *widget_ptr,
                               ATTR_UNUSED void *user_ptr) {
...
static void Change_Fullscreen_Resolution(EG_Widget *widget_ptr,
                                         void *user_ptr) {
  intptr_t value = (intptr_t)user_ptr;
...
static EG_BOOL Make_Video(SDL_Surface *dst_ptr) {
  ...
  widget_ptr = EG_Label_Create("screen:title", col, EG_LABEL_ALIGN_CENTER,
                               "Screen configuration:", loc);
  (void)EG_Window_AddWidget(window_ptr, widget_ptr);
...
#define SCREEN_CALC_LOC(c, r, b)                                               \
  loc = CalcRect(20 + c * (win.w / 2 - 20), 26 + (r + 1) * 17 + b * 10,        \
                 win.w / 2 - 20 - 5, 16)
  SCREEN_CALC_LOC(0, 0, 0);
  widget_ptr =
      EG_TickBox_Create("screen:low_grap", col, "Grille Graphics", loc);
  EG_Window_AddWidget(window_ptr, widget_ptr);
  (void)EG_TickBox_SetMyCallback_OnClick(
      widget_ptr, ProcessGUIOption,
  ...
  widget_ptr =
      EG_TickBox_Create("screen:low_text", col, "Grille Teletext", loc);
  EG_Window_AddWidget(window_ptr, widget_ptr);
  (void)EG_TickBox_SetMyCallback_OnClick(
      widget_ptr, ProcessGUIOption,
  ...
  widget_ptr =
      EG_TickBox_Create("screen:cursor", col, "Hide mouse pointer", loc);
  EG_Window_AddWidget(window_ptr, widget_ptr);
  (void)EG_TickBox_SetMyCallback_OnClick(
      widget_ptr, ProcessGUIOption,
  ...
  widget_ptr =
      EG_TickBox_Create("screen:keyboard_leds", col, "Show keyboard LEDs", loc);
  EG_Window_AddWidget(window_ptr, widget_ptr);
  (void)EG_TickBox_SetMyCallback_OnClick(
      widget_ptr, ProcessGUIOption,
  ...
  widget_ptr =
      EG_TickBox_Create("screen:disc_leds", col, "Show disc LEDs", loc);
  EG_Window_AddWidget(window_ptr, widget_ptr);
  (void)EG_TickBox_SetMyCallback_OnClick(
      widget_ptr, ProcessGUIOption,
  ...
  loc.w = 100;
  widget_ptr = EG_Label_Create("screen:fps:title", col, EG_LABEL_ALIGN_LEFT,
                               "Busy-wait:", loc);
  EG_Window_AddWidget(window_ptr, widget_ptr);
  ...
  (void)EG_ToggleButton_SetMyCallback_OnClick(
      widget_ptr, ProcessGUIOption,
  ...
  loc.w = 90;
  widget_ptr = EG_Label_Create("screen:leds:title", col, EG_LABEL_ALIGN_LEFT,
                               "LEDs are:", loc);
  EG_Window_AddWidget(window_ptr, widget_ptr);
  ...
  loc.w = 120;
  widget_ptr = EG_Label_Create("screen:monitor:title", col, EG_LABEL_ALIGN_LEFT,
                               "Monitor is:", loc);
  EG_Window_AddWidget(window_ptr, widget_ptr);
  ...
  loc.w *= 2;
  widget_ptr = EG_Label_Create("xxx", col, EG_LABEL_ALIGN_CENTER,
                               "Fullscreen Resolution", loc);
  EG_Window_AddWidget(window_ptr, widget_ptr);

  SCREEN_CALC_LOC(0, 8, 4);
  widget_ptr = EG_ToggleButton_Create("res:640x512", col, "640x512", loc);
  (void)EG_ToggleButton_SetMyCallback_OnClick(
      widget_ptr, Change_Fullscreen_Resolution, (void *)RESOLUTION_640X512);
  EG_RadioGroup_AddButton(group4_ptr, widget_ptr);
  gui.widget_fullscreen_640x512 = widget_ptr;

  SCREEN_CALC_LOC(1, 8, 4);
  widget_ptr =
      EG_ToggleButton_Create("res:640X480_S", col, "640x480 (scaled)", loc);
  (void)EG_ToggleButton_SetMyCallback_OnClick(
      widget_ptr, Change_Fullscreen_Resolution, (void *)RESOLUTION_640X480_S);
  EG_RadioGroup_AddButton(group4_ptr, widget_ptr);
  gui.widget_fullscreen_640x480_S = widget_ptr;

  SCREEN_CALC_LOC(0, 9, 4);
  widget_ptr =
      EG_ToggleButton_Create("res:640x480_V", col, "640x480 (centered)", loc);
  (void)EG_ToggleButton_SetMyCallback_OnClick(
      widget_ptr, Change_Fullscreen_Resolution, (void *)RESOLUTION_640X480_V);
  EG_RadioGroup_AddButton(group4_ptr, widget_ptr);
  gui.widget_fullscreen_640x480_V = widget_ptr;

  SCREEN_CALC_LOC(1, 9, 4);
  widget_ptr =
      EG_ToggleButton_Create("res:320X240_S", col, "320x240 (scaled)", loc);
  (void)EG_ToggleButton_SetMyCallback_OnClick(
      widget_ptr, Change_Fullscreen_Resolution, (void *)RESOLUTION_320X240_S);
  EG_RadioGroup_AddButton(group4_ptr, widget_ptr);
  gui.widget_fullscreen_320x240_S = widget_ptr;

  SCREEN_CALC_LOC(0, 10, 4);
  widget_ptr =
      EG_ToggleButton_Create("res:320x240_V", col, "320x240 (centered)", loc);
  (void)EG_ToggleButton_SetMyCallback_OnClick(
      widget_ptr, Change_Fullscreen_Resolution, (void *)RESOLUTION_320X240_V);
  EG_RadioGroup_AddButton(group4_ptr, widget_ptr);
  ...
  (void)EG_ToggleButton_SetMyCallback_OnClick(
      widget_ptr, Change_Fullscreen_Resolution, (void *)RESOLUTION_320X256);
  EG_RadioGroup_AddButton(group4_ptr, widget_ptr);
  ...
  loc.w *= 2;
  widget_ptr = EG_Label_Create("xxx", col, EG_LABEL_ALIGN_CENTER,
                               "Windowed Resolution", loc);
  EG_Window_AddWidget(window_ptr, widget_ptr);

  SCREEN_CALC_LOC(0, 12, 6);
  widget_ptr = EG_ToggleButton_Create("res:640x512", col, "640x512", loc);
  (void)EG_ToggleButton_SetMyCallback_OnClick(
      widget_ptr, Change_Windowed_Resolution, (void *)RESOLUTION_640X512);
  ...
  widget_ptr =
      EG_ToggleButton_Create("res:640X480_S", col, "640x480 (scaled)", loc);
  (void)EG_ToggleButton_SetMyCallback_OnClick(
      widget_ptr, Change_Windowed_Resolution, (void *)RESOLUTION_640X480_S);
  EG_RadioGroup_AddButton(group5_ptr, widget_ptr);
  ...
  widget_ptr =
      EG_ToggleButton_Create("res:640x480_V", col, "640x480 (centered)", loc);
  (void)EG_ToggleButton_SetMyCallback_OnClick(
      widget_ptr, Change_Windowed_Resolution, (void *)RESOLUTION_640X480_V);
  EG_RadioGroup_AddButton(group5_ptr, widget_ptr);
  ...
  widget_ptr =
      EG_ToggleButton_Create("res:320X240_S", col, "320x240 (scaled)", loc);
  (void)EG_ToggleButton_SetMyCallback_OnClick(
      widget_ptr, Change_Windowed_Resolution, (void *)RESOLUTION_320X240_S);
  EG_RadioGroup_AddButton(group5_ptr, widget_ptr);
  ...
  widget_ptr =
      EG_ToggleButton_Create("res:320x240_V", col, "320x240 (centered)", loc);
  (void)EG_ToggleButton_SetMyCallback_OnClick(
      widget_ptr, Change_Windowed_Resolution, (void *)RESOLUTION_320X240_V);
  EG_RadioGroup_AddButton(group5_ptr, widget_ptr);
  ...
  EG_RadioGroup_AddButton(group5_ptr, widget_ptr);
  (void)EG_ToggleButton_SetMyCallback_OnClick(
      widget_ptr, Change_Windowed_Resolution, (void *)RESOLUTION_320X256);
  ...
  widget_ptr = EG_Label_Create("screen:fps:title", col, EG_LABEL_ALIGN_LEFT,
  "FPS:",loc); EG_Window_AddWidget(window_ptr, widget_ptr);
  ...
  */
   ...
   */
  loc = CalcRect(win.w - 10 - 64, win.h - 10 - 20, 64, 20);
  widget_ptr = EG_Button_Create("but_screen_back", col, EG_BUTTON_ALIGN_CENTER,
                                "Back", loc);
...
static void Sound_Button_Back(ATTR_UNUSED EG_Widget *widget_ptr,
                              ATTR_UNUSED void *user_ptr) {
...
static EG_BOOL Make_Sound(SDL_Surface *dst_ptr) {
  ...
  /* Title label
   */
#define SOUND_CALC_LOC(r, b)                                                   \
  loc = CalcRect(20, 10 + r * 17 + b * 10, win.w - 40, 16)

  SOUND_CALC_LOC(0, 0);
  widget_ptr = EG_Label_Create("sound:sound:title", col, EG_LABEL_ALIGN_CENTER,
                               "Sound:", loc);
  (void)EG_Window_AddWidget(window_ptr, widget_ptr);
  ...
  SOUND_CALC_LOC(2, 0);
  widget_ptr =
      EG_TickBox_Create("sound:sound:yes/no", col, "Sound support", loc);
  EG_Window_AddWidget(window_ptr, widget_ptr);
  (void)EG_TickBox_SetMyCallback_OnClick(
      widget_ptr, ProcessGUIOption,
  ...
  SOUND_CALC_LOC(3, 0);
  widget_ptr =
      EG_TickBox_Create("sound:expvol", col, "Exponential Volume", loc);
  EG_Window_AddWidget(window_ptr, widget_ptr);
  (void)EG_TickBox_SetMyCallback_OnClick(
      widget_ptr, ProcessGUIOption,
  ...
  SOUND_CALC_LOC(4, 0);
  widget_ptr = EG_TickBox_Create("sound:latacy:yes/no", col,
                                 "Sound is low latency", loc);
  EG_Window_AddWidget(window_ptr, widget_ptr);
  (void)EG_TickBox_SetMyCallback_OnClick(
      widget_ptr, ProcessGUIOption,
  ...
  SOUND_CALC_LOC(5, 0);
  widget_ptr =
      EG_TickBox_Create("sound:speech:yes/no", col, "Speech support", loc);
  EG_Window_AddWidget(window_ptr, widget_ptr);
  (void)EG_TickBox_SetMyCallback_OnClick(
      widget_ptr, ProcessGUIOption,
  ...
  SOUND_CALC_LOC(6, 0);
  widget_ptr = EG_TickBox_Create("sound:Relay:yes/no", col,
                                 "Cassette relay effects", loc);
  EG_Window_AddWidget(window_ptr, widget_ptr);
  (void)EG_TickBox_SetMyCallback_OnClick(
      widget_ptr, ProcessGUIOption,
  ...
  SOUND_CALC_LOC(7, 0);
  widget_ptr = EG_TickBox_Create("sound:Input:yes/no", col,
                                 "Cassette input effects", loc);
  EG_Window_AddWidget(window_ptr, widget_ptr);
  (void)EG_TickBox_SetMyCallback_OnClick(
      widget_ptr, ProcessGUIOption,
  ...
  SOUND_CALC_LOC(9, 0);
  widget_ptr = EG_Label_Create("sound:quality:title", col,
                               EG_LABEL_ALIGN_CENTER, "Quality:", loc);
  EG_Window_AddWidget(window_ptr, widget_ptr);
  ...
  SOUND_CALC_LOC(11, 0);
  widget_ptr =
      EG_RadioButton_Create("sound:quality:11_025", col, "11.025 kHz", loc);
  EG_RadioGroup_AddButton(group1_ptr, widget_ptr);
  (void)EG_TickBox_SetMyCallback_OnClick(
      widget_ptr, ProcessGUIOption,
  ...
  SOUND_CALC_LOC(12, 0);
  widget_ptr =
      EG_RadioButton_Create("sound:quality:22_050", col, "22.050 kHz", loc);
  EG_RadioGroup_AddButton(group1_ptr, widget_ptr);
  //	EG_RadioGroup_Select(widget_ptr);
  (void)EG_TickBox_SetMyCallback_OnClick(
      widget_ptr, ProcessGUIOption,
  ...
  SOUND_CALC_LOC(13, 0);
  widget_ptr =
      EG_RadioButton_Create("sound:quality:44_100", col, "44.100 kHz", loc);
  EG_RadioGroup_AddButton(group1_ptr, widget_ptr);
  (void)EG_TickBox_SetMyCallback_OnClick(
      widget_ptr, ProcessGUIOption,
  ...
  SOUND_CALC_LOC(15, 0);
  widget_ptr = EG_Label_Create("sound:volume:label", col, EG_LABEL_ALIGN_CENTER,
                               "Volume:", loc);
  EG_Window_AddWidget(window_ptr, widget_ptr);
  ...
  SOUND_CALC_LOC(17, 0);
  widget_ptr =
      EG_RadioButton_Create("sound:volume:full", col, "Full volume", loc);
  EG_RadioGroup_AddButton(group2_ptr, widget_ptr);
  (void)EG_RadioButton_SetMyCallback_OnClick(
      widget_ptr, ProcessGUIOption,
      CreateWindowsMenuItem(IDM_FULLVOLUME, widget_ptr));

  SOUND_CALC_LOC(18, 0);
  widget_ptr =
      EG_RadioButton_Create("sound:volume:high", col, "High volume", loc);
  EG_RadioGroup_AddButton(group2_ptr, widget_ptr);
  (void)EG_RadioButton_SetMyCallback_OnClick(
      widget_ptr, ProcessGUIOption,
  ...
  SOUND_CALC_LOC(19, 0);
  widget_ptr =
      EG_RadioButton_Create("sound:volume:medium", col, "Medium volume", loc);
  EG_RadioGroup_AddButton(group2_ptr, widget_ptr);
  (void)EG_RadioButton_SetMyCallback_OnClick(
      widget_ptr, ProcessGUIOption,
  ...
  SOUND_CALC_LOC(20, 0);
  widget_ptr =
      EG_RadioButton_Create("sound:volume:low", col, "Low volume", loc);
  EG_RadioGroup_AddButton(group2_ptr, widget_ptr);
  (void)EG_RadioButton_SetMyCallback_OnClick(
      widget_ptr, ProcessGUIOption,
   ...
   */
  loc = CalcRect(win.w - 10 - 64, win.h - 10 - 20, 64, 20);
  widget_ptr = EG_Button_Create("but_sound_back", col, EG_BUTTON_ALIGN_CENTER,
                                "Back", loc);
...
static void ROMs_Button_Back(ATTR_UNUSED EG_Widget *widget_ptr,
                             ATTR_UNUSED void *user_ptr) {
...
static EG_BOOL Make_ROMs(SDL_Surface *dst_ptr) {
  ...
  /* Title label
   */
#define ROMS_CALC_LOC(r, b)                                                    \
  loc = CalcRect(20, 10 + r * 17 + b * 10, win.w - 40, 16)

  ROMS_CALC_LOC(0, 0);
  widget_ptr = EG_Label_Create("lab_sound", col, EG_LABEL_ALIGN_CENTER,
                               "Allow ROM writes:", loc);
  (void)EG_Window_AddWidget(window_ptr, widget_ptr);
  ...
  //	ROMS_CALC_LOC(2, 0);
  //	widget_ptr = EG_Label_Create("lab_caption", col, EG_LABEL_ALIGN_LEFT,
  //"R/W", loc); 	(void) EG_Window_AddWidget(window_ptr, widget_ptr);
   ...
   */
  loc = CalcRect(win.w - 10 - 64, win.h - 10 - 20, 64, 20);
  widget_ptr = EG_Button_Create("but_roms_back", col, EG_BUTTON_ALIGN_CENTER,
                                "Back", loc);
...
static void Speed_Button_Back(ATTR_UNUSED EG_Widget *widget_ptr,
                              ATTR_UNUSED void *user_ptr) {
...
#define SPEED_COUNT 14
  float speed[] = {100, 50, 10,  5,    2,   1.5,  1.25,
                   1.1, 1,  0.9, 0.75, 0.5, 0.25, 0.1};
  int speed_menu_id[] = {
  ...
  /* Title label
   */
#define SPEED_CALC_LOC(r, b)                                                   \
  loc = CalcRect(20, 10 + r * 17 + b * 10, win.w - 40, 16)

  SPEED_CALC_LOC(0, 0);
  loc.x -= 10;
  loc.w += 20;
  widget_ptr = EG_Label_Create("speed:title", col, EG_LABEL_ALIGN_CENTER,
                               "Emulator speed:", loc);
  (void)EG_Window_AddWidget(window_ptr, widget_ptr);
   ...
   */
  loc = CalcRect(win.w - 10 - 64, win.h - 10 - 20, 64, 20);
  widget_ptr = EG_Button_Create("but_speed_back", col, EG_BUTTON_ALIGN_CENTER,
                                "Back", loc);
...
static void About_Slidebar(EG_Widget *widget_ptr, void *user_ptr)
{
        ...
        unsigned long index = EG_SlideBar_GetPosition(gui.widget_about_slider)
/16; int i;
...
//gui.widget_about
        if (index != previous_index){
                ...
                for(i=0; i<24; i++){
                        EG_Label_SetCaption(gui.widget_about[i],
about[i+index]);
                }
        }
...
#	define ABOUT_CALC_LOC(r, b) loc = CalcRect(20, 10+r*16+b*10, win.w-40,
16)

        ABOUT_CALC_LOC(0, 0);
        widget_ptr = EG_Label_Create("lab_about", col, EG_LABEL_ALIGN_CENTER,
"Beebem - BBC Model B / Master 128 Emulator", loc); (void)
EG_Window_AddWidget(window_ptr, widget_ptr);
        ...
        for(i=0; i<24; i++){
                ...
                widget_ptr = EG_Label_Create(NULL, col, EG_LABEL_ALIGN_LEFT,
about[i], loc); (void) EG_Window_AddWidget(window_ptr, widget_ptr); loc.y+=16;
        ...
        widget_ptr = EG_Button_Create("but_about_back", col,
EG_BUTTON_ALIGN_CENTER, "Back", loc); (void)
EG_Button_SetMyCallback_OnClick(widget_ptr, About_Button_Back, &gui); (void)
EG_Window_AddWidget(window_ptr, widget_ptr); (void)
...
static void Devices_Button_Back(ATTR_UNUSED EG_Widget *widget_ptr,
                                ATTR_UNUSED void *user_ptr) {
  //	int count = 0;
 ...
 */
  ...
  /* Title label
   */
#define DEVICES_CALC_LOC(r, b)                                                 \
  loc = CalcRect(20, 10 + r * 17 + b * 8, win.w - 40, 16)

  DEVICES_CALC_LOC(0, 0);
  widget_ptr = EG_Label_Create("lab_devices", col, EG_LABEL_ALIGN_CENTER,
                               "Cheese Wedge:", loc);
  (void)EG_Window_AddWidget(window_ptr, widget_ptr);
  ...
  loc.x += loc.w + 10;
  widget_ptr =
      EG_ToggleButton_Create("system:tube:TorchZ80", col, "Torch Z80", loc);
  EG_Window_AddWidget(window_ptr, widget_ptr);
  (void)EG_ToggleButton_SetMyCallback_OnClick(
      widget_ptr, ProcessGUIOption,
  ...
  loc.w -= 5;
  widget_ptr =
      EG_ToggleButton_Create("system:tube:AcornZ80", col, "Acorn Z80", loc);
  EG_Window_AddWidget(window_ptr, widget_ptr);
  (void)EG_ToggleButton_SetMyCallback_OnClick(
      widget_ptr, ProcessGUIOption,
  ...
  loc.x += loc.w + 10;
  widget_ptr =
      EG_ToggleButton_Create("system:tube:i86", col, "Master 512 80186", loc);
  EG_Window_AddWidget(window_ptr, widget_ptr);
  (void)EG_ToggleButton_SetMyCallback_OnClick(
      widget_ptr, ProcessGUIOption,
  ...
  if (peripherals_count > 0) {
    ...
    DEVICES_CALC_LOC(5, 0);
    widget_ptr = EG_Label_Create("system:network:title", col,
                                 EG_LABEL_ALIGN_CENTER, "Devices:", loc);
    EG_Window_AddWidget(window_ptr, widget_ptr);
    ...
    loc.w += 20;
    widget_ptr =
        EG_Box_Create("system:network:box", EG_BOX_BORDER_SUNK, col, loc);
    EG_Window_AddWidget(window_ptr, widget_ptr);
    ...
    DEVICES_CALC_LOC(y, 0);
    widget_ptr =
        EG_TickBox_Create("per:harddrive", col, "Hard drives active", loc);
    EG_Window_AddWidget(window_ptr, widget_ptr);
    (void)EG_TickBox_SetMyCallback_OnClick(
        widget_ptr, ProcessGUIOption,
    ...
    DEVICES_CALC_LOC(y, 0);
    widget_ptr = EG_TickBox_Create("per:touchscreen", col,
                                   "Microvitec Touch Screen active", loc);
    EG_Window_AddWidget(window_ptr, widget_ptr);
    (void)EG_TickBox_SetMyCallback_OnClick(
        widget_ptr, ProcessGUIOption,
    ...
    /*		DEVICES_CALC_LOC(y, 0);
                    widget_ptr = EG_TickBox_Create("per:teletext", col,
       "Teletext adapter active", loc); EG_Window_AddWidget(window_ptr,
       widget_ptr); (void) EG_TickBox_SetMyCallback_OnClick(widget_ptr,
       ProcessGUIOption , CreateWindowsMenuItem(ID_TELETEXT, widget_ptr)); y++;
    */
  }
   ...
   */
  loc = CalcRect(win.w - 10 - 64, win.h - 10 - 20, 64, 20);
  widget_ptr = EG_Button_Create("but_devices_back", col, EG_BUTTON_ALIGN_CENTER,
                                "Back", loc);
...
static void Tapes_Button_Back(ATTR_UNUSED EG_Widget *widget_ptr,
                              ATTR_UNUSED void *user_ptr) {
...
static EG_BOOL Make_Tapes(SDL_Surface *dst_ptr) {
  ...
  /* Title label
   */
#define TAPES_CALC_LOC(r, b)                                                   \
  loc = CalcRect(20, 10 + r * 20 + b * 10, win.w - 40, 19)

  TAPES_CALC_LOC(0, 0);
  widget_ptr = EG_Label_Create("lab_tapes", col, EG_LABEL_ALIGN_CENTER,
                               "Tape configuration:", loc);
  (void)EG_Window_AddWidget(window_ptr, widget_ptr);
  ...
  /* Insert tape: */
  TAPES_CALC_LOC(9, 0);
  widget_ptr = EG_Button_Create("disks:LoadTape", col, EG_BUTTON_ALIGN_CENTER,
                                "Insert tape", loc);
  EG_Window_AddWidget(window_ptr, widget_ptr);
  (void)EG_TickBox_SetMyCallback_OnClick(
      widget_ptr, ProcessGUIOption,
  ...
  TAPES_CALC_LOC(10, 0);
  widget_ptr = EG_Button_Create("disks:Rewind", col, EG_BUTTON_ALIGN_CENTER,
                                "Rewind tape", loc);
  EG_Window_AddWidget(window_ptr, widget_ptr);
  (void)EG_TickBox_SetMyCallback_OnClick(
      widget_ptr, ProcessGUIOption,
   ...
   */
  loc = CalcRect(win.w - 10 - 64, win.h - 10 - 20, 64, 20);
  widget_ptr = EG_Button_Create("but_tapes_back", col, EG_BUTTON_ALIGN_CENTER,
                                "Back", loc);
...
static void Disks_Button_Back(ATTR_UNUSED EG_Widget *widget_ptr,
                              ATTR_UNUSED void *user_ptr) {
...
static EG_BOOL Make_Disks(SDL_Surface *dst_ptr) {
  ...
  /* Title label
   */
#define DISKS_CALC_LOC(r, b)                                                   \
  loc = CalcRect(20, 10 + r * 20 + b * 10, win.w - 40, 19)

  DISKS_CALC_LOC(0, 0);
  widget_ptr =
      EG_Label_Create("lab_disks", col, EG_LABEL_ALIGN_CENTER, "Disks:", loc);
  (void)EG_Window_AddWidget(window_ptr, widget_ptr);

  /* Back button
   */
  loc = CalcRect(win.w - 10 - 64, win.h - 10 - 20, 64, 20);
  widget_ptr = EG_Button_Create("but_disks_back", col, EG_BUTTON_ALIGN_CENTER,
                                "Back", loc);
...
static void Keyboard_Button_Back(ATTR_UNUSED EG_Widget *widget_ptr,
                                 ATTR_UNUSED void *user_ptr) {
...
static void Disks_SaveRegistry(ATTR_UNUSED EG_Widget *widget_ptr,
                               ATTR_UNUSED void *user_ptr) {
  ...
  if (SaveFakeRegistry() == FALSE) {
    ...
    EG_MessageBox(screen_ptr, EG_MESSAGEBOX_STOP, "Failed save",
                  "Could not save registry! Save failed!", "OK", NULL, NULL,
                  NULL, 0);
  }
}
...
static EG_BOOL Make_Keyboard(SDL_Surface *dst_ptr) {
  ...
  /* Title label
   */
#define KEYBOARD_CALC_LOC(r, b)                                                \
  loc = CalcRect(20, 10 + r * 20 + b * 10, win.w - 40, 16)

  KEYBOARD_CALC_LOC(0, 0);
  widget_ptr = EG_Label_Create("lab_keyboard", col, EG_LABEL_ALIGN_CENTER,
                               "Keyboard:", loc);
  (void)EG_Window_AddWidget(window_ptr, widget_ptr);

  DISKS_CALC_LOC(3, 0);
  widget_ptr =
      EG_Button_Create("disks:Drive:run_disc", col, EG_BUTTON_ALIGN_CENTER,
                       "Select, load and run a disc", loc);
  (void)EG_Button_SetMyCallback_OnClick(widget_ptr, RunDisc, window_ptr);
  (void)EG_Window_AddWidget(window_ptr, widget_ptr);

  DISKS_CALC_LOC(4, 0);
  widget_ptr = EG_Button_Create("disks:Drive0:load_state", col,
                                EG_BUTTON_ALIGN_CENTER, "Load state", loc);
  (void)EG_Button_SetMyCallback_OnClick(widget_ptr, LoadState, window_ptr);
  (void)EG_Window_AddWidget(window_ptr, widget_ptr);

  DISKS_CALC_LOC(5, 0);
  widget_ptr =
      EG_Button_Create("disks:Drive0:load_disk", col, EG_BUTTON_ALIGN_CENTER,
                       "Change disk in drive 0", loc);
  EG_Window_AddWidget(window_ptr, widget_ptr);
  (void)EG_TickBox_SetMyCallback_OnClick(
      widget_ptr, ProcessGUIOption,
  ...
  DISKS_CALC_LOC(6, 0);
  widget_ptr =
      EG_Button_Create("disks:Drive1:load_disk", col, EG_BUTTON_ALIGN_CENTER,
                       "Change disk in drive 1", loc);
  EG_Window_AddWidget(window_ptr, widget_ptr);
  (void)EG_TickBox_SetMyCallback_OnClick(
      widget_ptr, ProcessGUIOption,
  ...
  DISKS_CALC_LOC(8, 0);
  widget_ptr =
      EG_Button_Create("disks:Drive0:new_disk", col, EG_BUTTON_ALIGN_CENTER,
                       "Create new disc image in drive 0", loc);
  EG_Window_AddWidget(window_ptr, widget_ptr);
  (void)EG_TickBox_SetMyCallback_OnClick(
      widget_ptr, ProcessGUIOption,
  ...
  DISKS_CALC_LOC(9, 0);
  widget_ptr =
      EG_Button_Create("disks:Drive1:new_disk", col, EG_BUTTON_ALIGN_CENTER,
                       "Create new disc image in drive 1", loc);
  EG_Window_AddWidget(window_ptr, widget_ptr);
  (void)EG_TickBox_SetMyCallback_OnClick(
      widget_ptr, ProcessGUIOption,
  ...
  widget_ptr = EG_Button_Create("disks:Drive0:save_state", col,
                                EG_BUTTON_ALIGN_CENTER, "Save state", loc);
  (void)EG_Button_SetMyCallback_OnClick(widget_ptr, SaveState, window_ptr);
  (void)EG_Window_AddWidget(window_ptr, widget_ptr);

  DISKS_CALC_LOC((10 + 3), 0);
  widget_ptr =
      EG_TickBox_Create("disks:WriteProtect0:yes/no", col,
                        "Disk 0: Read only (click below to eject)", loc);
  EG_Window_AddWidget(window_ptr, widget_ptr);
  (void)EG_TickBox_SetMyCallback_OnClick(
      widget_ptr, ProcessGUIOption,
  ...
  widget_ptr = EG_Button_Create("disks:Eject:Disc0", col,
                                EG_BUTTON_ALIGN_CENTER, "", loc);
  EG_Window_AddWidget(window_ptr, widget_ptr);
  (void)EG_Button_SetMyCallback_OnClick(
      widget_ptr, ProcessGUIOption,
  ...
  widget_ptr =
      EG_TickBox_Create("disks:WriteProtect1:yes/no", col,
                        "Disk 1: Read only (click below to eject)", loc);
  EG_Window_AddWidget(window_ptr, widget_ptr);
  (void)EG_TickBox_SetMyCallback_OnClick(
      widget_ptr, ProcessGUIOption,
  ...
  widget_ptr = EG_Button_Create("disks:Eject:Disc1", col,
                                EG_BUTTON_ALIGN_CENTER, "", loc);
  EG_Window_AddWidget(window_ptr, widget_ptr);
  (void)EG_Button_SetMyCallback_OnClick(
      widget_ptr, ProcessGUIOption,
  ...
  widget_ptr =
      EG_Label_Create("disks:lab_1", col, EG_LABEL_ALIGN_LEFT,
                      "Watch out for GTK opening the file selector", loc);
  (void)EG_Window_AddWidget(window_ptr, widget_ptr);

  DISKS_CALC_LOC((18 + 1), 0);
  loc.y--;
  widget_ptr =
      EG_Label_Create("disks:lab_2", col, EG_LABEL_ALIGN_LEFT,
                      "behind the emulator window!                ", loc);
  (void)EG_Window_AddWidget(window_ptr, widget_ptr);
  col = MENU_COLORS; // CalcColor(191+32, 191+32, 0);

  /*
          DISKS_CALC_LOC(15, 0);
          widget_ptr = EG_Button_Create("disks:nativeFDC", col,
     EG_BUTTON_ALIGN_CENTER, "Native 8271", loc); (void)
     EG_Window_AddWidget(window_ptr, widget_ptr); (void)
     EG_TickBox_SetMyCallback_OnClick(widget_ptr, Disks_NativeDFS, NULL);

          DISKS_CALC_LOC(16, 0);
          widget_ptr = EG_Button_Create("disks:acorn", col,
     EG_BUTTON_ALIGN_CENTER, "Acorn 1770 DFS Board Drive Controller", loc);
          (void) EG_Window_AddWidget(window_ptr, widget_ptr);
          (void) EG_TickBox_SetMyCallback_OnClick(widget_ptr, Disks_AcornDFS,
     NULL);

          DISKS_CALC_LOC(17, 0);
          widget_ptr = EG_Button_Create("disks:watford", col,
     EG_BUTTON_ALIGN_CENTER, "Watford DDFS Ext. board for Model B", loc); (void)
     EG_Window_AddWidget(window_ptr, widget_ptr); (void)
     EG_TickBox_SetMyCallback_OnClick(widget_ptr, Disks_WatfordDFS, NULL);

          DISKS_CALC_LOC(18, 0);
          widget_ptr = EG_Button_Create("disks:opus", col,
     EG_BUTTON_ALIGN_CENTER, "Opus DDFS Board Drive Controller", loc); (void)
     EG_Window_AddWidget(window_ptr, widget_ptr); (void)
     EG_TickBox_SetMyCallback_OnClick(widget_ptr, Disks_OpusDFS, NULL);

          DISKS_CALC_LOC(19, 0); loc.y--;
          widget_ptr = EG_Label_Create("disks:fdc_label", col,
     EG_LABEL_ALIGN_CENTER, "", loc); (void) EG_Window_AddWidget(window_ptr,
     widget_ptr); gui.widget_fdc_label = widget_ptr;

  */

  DISKS_CALC_LOC(2, 0);
  loc.y -= 8;
  widget_ptr = EG_Label_Create("disks:lab_2", col, EG_LABEL_ALIGN_CENTER,
                               "(this mess is only temporary!)", loc);
  (void)EG_Window_AddWidget(window_ptr, widget_ptr);

  DISKS_CALC_LOC(20, 0);
  widget_ptr =
      EG_Button_Create("disks:save_config", col, EG_BUTTON_ALIGN_CENTER,
                       "Save configuration", loc);
  (void)EG_Window_AddWidget(window_ptr, widget_ptr);
  ...
  //      widget_ptr = EG_Box_Create("system_model_box", EG_BOX_BORDER_SUNK,
  //      col, loc);
  //    EG_Window_AddWidget(window_ptr, widget_ptr);
  ...
  //	SYSTEM_CALC_LOC(3, 0);
  //	widget_ptr = EG_RadioButton_Create("model_b_and_integra_b", col, "BBC
  // Model B + Integra-B", loc); 	EG_RadioButton_Disable(widget_ptr);
  //	EG_RadioGroup_AddButton(group1_ptr, widget_ptr);

  //	SYSTEM_CALC_LOC(4, 0);
  //	widget_ptr = EG_RadioButton_Create("model_b_plus", col, "BBC Model B
  // Plus", loc); 	EG_RadioButton_Disable(widget_ptr);
  //	EG_RadioGroup_AddButton(group1_ptr, widget_ptr);

  //	SYSTEM_CALC_LOC(5, 0);
  //	widget_ptr = EG_RadioButton_Create("model_master_128", col, "BBC Master
  // 128", loc); 	EG_RadioGroup_AddButton(group1_ptr, widget_ptr);

  //      (void) EG_Window_AddWidget(window_ptr, group1_ptr);

  //	SYSTEM_CALC_LOC(19, 0);
  //	widget_ptr = EG_Label_Create("lab_econet", col, EG_LABEL_ALIGN_CENTER,
  //"Network:", loc); 	EG_Window_AddWidget(window_ptr, widget_ptr);
...
static void AMX_Button_Back(ATTR_UNUSED EG_Widget *widget_ptr,
                            ATTR_UNUSED void *user_ptr) {
...
static EG_BOOL Make_AMX(SDL_Surface *dst_ptr) {
  ...
  /* Title label
   */
#define AMX_CALC_LOC(r, b)                                                     \
  loc = CalcRect(20, 10 + r * 20 + b * 10, win.w - 40, 16)

  AMX_CALC_LOC(0, 0);
  widget_ptr =
      EG_Label_Create("lab_amx", col, EG_LABEL_ALIGN_CENTER, "AMX:", loc);
  (void)EG_Window_AddWidget(window_ptr, widget_ptr);
  ...
  AMX_CALC_LOC(4, 2);
  widget_ptr =
      EG_RadioButton_Create("amx:map160x256", col, "Map to 160x256", loc);
  EG_RadioGroup_AddButton(group1_ptr, widget_ptr);
  (void)EG_TickBox_SetMyCallback_OnClick(
      widget_ptr, ProcessGUIOption,
  ...
  AMX_CALC_LOC(5, 2);
  widget_ptr =
      EG_RadioButton_Create("amx:map320x256", col, "Map to 320x256", loc);
  EG_RadioGroup_AddButton(group1_ptr, widget_ptr);
  (void)EG_TickBox_SetMyCallback_OnClick(
      widget_ptr, ProcessGUIOption,
  ...
  AMX_CALC_LOC(6, 2);
  widget_ptr =
      EG_RadioButton_Create("amx:map540x256", col, "Map to 640x256", loc);
  EG_RadioGroup_AddButton(group1_ptr, widget_ptr);
  (void)EG_TickBox_SetMyCallback_OnClick(
      widget_ptr, ProcessGUIOption,
   ...
   */
  loc = CalcRect(win.w - 10 - 64, win.h - 10 - 20, 64, 20);
  widget_ptr = EG_Button_Create("but_amx_back", col, EG_BUTTON_ALIGN_CENTER,
                                "Back", loc);
